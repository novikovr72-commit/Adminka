...
import OrganizationStructureEmployeesPanel from "./components/OrganizationStructureEmployeesPanel";
import EmployeesListTableRow from "./components/EmployeesListTableRow";
import EmployeeRelationsListTableRow from "./components/EmployeeRelationsListTableRow";
import ListTableVirtualRows from "./components/ListTableVirtualRows";
import OrganizationListTableRow from "./components/OrganizationListTableRow";
import ReferenceBooksListPage from "./components/ReferenceBooksListPage";
import ReferenceBookDataPage from "./components/ReferenceBookDataPage";
import { MIN_COLUMN_WIDTH, fitListColumnsToTargetWidth } from "./listColumnFit";
import {
  invokeReferenceBooksAlignColumns,
  invokeReferenceBooksClearFilters,
  invokeReferenceBooksOpenAdd,
  invokeReferenceBooksOpenColumnSettings
} from "./referenceBooksHeaderHandlers";
import {
  invokeReferenceBookDataAlignColumns,
  invokeReferenceBookDataClearFilters,
  invokeReferenceBookDataExportFile,
  invokeReferenceBookDataOpenAdd,
  invokeReferenceBookDataOpenColumnSettings
} from "./referenceBookDataHeaderHandlers";

const ReportSettingsPage = lazy(() => import("./components/ReportSettingsPage"));
const OrganizationStructureEditorModal = lazy(() => import("./components/OrganizationStructureEditorModal"));
import {
  ...
  REPORT_TEMPLATE_FIELDS_MIN_COLUMN_WIDTH_PX,
  REPORT_TEMPLATE_GENERAL_SETTINGS_COLUMNS,
  REPORT_TEMPLATE_GENERAL_SETTINGS_DEFAULT_PARAMETER_COL_PX,
  REPORT_TEMPLATE_GENERAL_SETTINGS_MIN_PARAMETER_COL_PX,
  REPORT_TEMPLATE_GENERAL_SETTINGS_MIN_VALUE_COL_PX,
  REPORT_TEMPLATE_GENERAL_SETTINGS_ROWS,
...
import {
  REPORT_ACCESS_GROUP_ENUM,
  REPORT_CARD_TABS,
  REPORT_SQL_BASE_FONT_SIZE_PX,
  REPORT_SQL_EDITOR_LINE_HEIGHT_PX,
  REPORT_SQL_EDITOR_PADDING_PX,
  REPORT_SQL_RESULTS_PREVIEW_LIMIT,
  REPORT_SQL_VIEW_MODES,
  REPORT_TEMPLATE_VIEW_MODES
} from "./reportCardConstants";
import TableColumnSettingsPanel from "./components/TableColumnSettingsPanel";
import {
  LIST_ROW_ESTIMATE_PX,
  LIST_VIRTUALIZATION_PAGE_SIZE_THRESHOLD,
  LIST_VIRTUALIZATION_ROW_COUNT_THRESHOLD
} from "./mainListVirtualConstants";
import {
  IconAlignColumns,
  IconArrowRight,
  IconBuilding,
...
/** Предзагрузка чанка JSON-редактора вместе с вкладкой «Шаблон». */
function prefetchReportTemplateJsonEditorPanel() {
  void import("./components/ReportTemplateJsonEditorPanel");
}

function prefetchReportSettingsPage() {
  void import("./components/ReportSettingsPage");
}
...
function escapeHtmlForSqlHighlight(text) {
  return String(text)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
let prismSqlModulePromise = null;
function loadPrismSqlModule() {
  if (!prismSqlModulePromise) {
    prismSqlModulePromise = import("prismjs").then((prismMod) =>
      import("prismjs/components/prism-sql").then(() => prismMod.default ?? prismMod)
    );
  }
  return prismSqlModulePromise;
}

/** Предзагрузка Prism + грамматики SQL при переходе на вкладку — меньше паузы при первом рендере подсветки. */
function prefetchPrismSqlHighlighter() {
  void loadPrismSqlModule();
}
...
const ORGANIZATION_CARD_RELATION_COLUMN_WIDTHS_STORAGE_KEY = "organization-card.relation-column-widths.v1";
const ORGANIZATION_CARD_RELATION_SORT_RULES_STORAGE_KEY = "organization-card.relation-sort-rules.v1";
const ORGANIZATION_CARD_RELATION_FILTERS_STORAGE_KEY = "organization-card.relation-filters.v1";
...
const ORGANIZATION_STRUCTURE_COLUMN_FRACS_STORAGE_KEY = "organization-card.structure-column-fracs.v1";
const ORGANIZATION_STRUCTURE_PANES_SPLIT_STORAGE_KEY = "organization-card.structure-panes-split.v1";
const ORGANIZATION_STRUCTURE_PANES_HANDLE_PX = 10;
const ORGANIZATION_STRUCTURE_PANES_MIN_LEFT_PX = 200;
const ORGANIZATION_STRUCTURE_PANES_MIN_RIGHT_PX = 280;
...
/** UI с localhost/127.0.0.1 + .env с localhost:3003 → иначе fetch на «localhost:3003» при странице 127.0.0.1 часто даёт Failed to fetch */
const IS_LOCAL_PAGE_HOST =
  IS_BROWSER &&
  ["localhost", "127.0.0.1"].includes(String(window.location.hostname ?? "").toLowerCase());
const CONFIGURED_USES_DEFAULT_ADMIN_PORT =
  !CONFIGURED_API_BASE_URL ||
  /:\s*3003(\/|$)/.test(CONFIGURED_API_BASE_URL) ||
  /^https?:\/\/(localhost|127\.0\.0\.1)\/?$/i.test(CONFIGURED_API_BASE_URL);
const USE_RUNTIME_FOR_LOCAL_DEV =
  IS_LOCALHOST_CONFIG &&
  IS_LOCAL_PAGE_HOST &&
  CONFIGURED_USES_DEFAULT_ADMIN_PORT;
const API_BASE_URL = (
  IS_REMOTE_BROWSER && IS_LOCALHOST_CONFIG
    ? RUNTIME_API_BASE_URL
    : USE_RUNTIME_FOR_LOCAL_DEV
    ? RUNTIME_API_BASE_URL
...
const REFERENCE_BOOKS_LIST_API_URL = `${ADMIN_API_BASE_URL}/reference-books`;
...
const ORGANIZATION_DESCENDANTS_API_PATH = (organUnitId) =>
  `${ADMIN_API_BASE_URL}/organization/${organUnitId}/descendants`;
const ORGANIZATION_SUBDIVISIONS_CREATE_API_PATH = (parentOrganUnitId) =>
  `${ADMIN_API_BASE_URL}/organization/${encodeURIComponent(parentOrganUnitId)}/subdivisions`;
const ORGANIZATION_PARENT_PATCH_API_PATH = (organUnitId) =>
  `${ADMIN_API_BASE_URL}/organization/${encodeURIComponent(organUnitId)}/parent`;
const ORGANIZATION_STRUCTURE_ACTIONS_COL_PX = 88;
const ORGANIZATION_STRUCTURE_MIN_DATA_COL_PX = 64;
const ORGANIZATION_STRUCTURE_WRAPPER_GUTTER_PX = 6;
/** Подсветка drop на строке «организация (корень карточки)» */
const ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY = "__structure_root_drop__";
...
const PAGE_IDS = {
  ...
  REFERENCE_BOOK_DATA: "reference-book-data"
};
const PAGE_TITLES = {
  ...
  [PAGE_IDS.REFERENCE_BOOKS]: "Список справочников",
  [PAGE_IDS.REFERENCE_BOOK_DATA]: "Данные справочника"
};
const PAGE_PATHNAMES = {
  ...
  [PAGE_IDS.REFERENCE_BOOKS]: "/reference-books"
};
...
const ORGANIZATION_RELATION_COLUMNS = [
  { key: "employeeName", title: "ФИО сотрудника" },
  { key: "relationName", title: "Тип отношения" },
  { key: "defaultFlag", title: "Основное отношение" },
  { key: "salesOrganName", title: "Сбытовая организация" },
  { key: "productGroupName", title: "Группа продуктов" }
];
const DEFAULT_ORGANIZATION_RELATION_COLUMN_WIDTHS = {
  employeeName: 240,
  relationName: 220,
  defaultFlag: 160,
  salesOrganName: 220,
...
const INITIAL_ORGANIZATION_RELATION_FILTERS = {
  employeeName: "",
  relationName: "",
  defaultFlag: "",
  salesOrganName: "",
...
const INITIAL_NEW_POSITION_FORM = {
  ...
  departName: "",
  departUnitId: "",
  /** Подразделение записи на момент открытия редактирования (сравнение с правилами карточки организации) */
  editInitialDepartUnitId: "",
  positionName: "",
  employeePositionId: "",
  bossName: "",
  bossEmployeeId: "",
...
/** Сегмент после /reference-books/settings/ — суффикс URL для карточки настроек справочника. */
const getReferenceBookSettingsSuffixFromUrl = () => {
  if (typeof window === "undefined") {
    return "";
  }
  const normalized = normalizePathname(window.location.pathname);
  const match = /^\/reference-books\/settings\/([^/]+)$/i.exec(normalized);
  if (!match) {
    return "";
  }
  try {
...
/** Сегмент пути после /reference-books/ — данные справочника (не /reference-books/settings/...). */
const getReferenceBookDataSuffixFromUrl = () => {
  if (typeof window === "undefined") {
    return "";
  }
  const normalized = normalizePathname(window.location.pathname);
  if (/^\/reference-books\/settings\//i.test(normalized)) {
    return "";
  }
  const match = /^\/reference-books\/([^/]+)$/i.exec(normalized);
  if (!match) {
    return "";
  }
  try {
...
const getPageIdFromUrl = () => {
  ...
  const normalized = normalizePathname(window.location.pathname);
  if (getReferenceBookDataSuffixFromUrl()) {
    return PAGE_IDS.REFERENCE_BOOK_DATA;
  }
  if (/^\/reference-books\/settings\//i.test(normalized)) {
    return PAGE_IDS.REFERENCE_BOOKS;
  }
...
const normalizeOrganUnitDescendantEmployee = (raw) => ({
  employeeOrganId: String(raw?.employeeOrganId ?? raw?.employee_organ_id ?? "").trim(),
  employeeId: String(raw?.employeeId ?? raw?.employee_id ?? "").trim(),
  employeeParentId: String(raw?.employeeParentId ?? raw?.employee_parent_id ?? "").trim(),
  employeeFullName: normalizeUiText(raw?.employeeFullName ?? raw?.employee_full_name),
  employeePositionName: normalizeUiText(raw?.employeePositionName ?? raw?.employee_position_name),
  employeePositionId: String(raw?.employeePositionId ?? raw?.employee_position_id ?? "").trim()
});

const normalizeOrganStructureItem = (raw) => ({
  organUnitChildId: String(raw?.organUnitChildId ?? raw?.organ_unit_child_id ?? "").trim(),
  organUnitName: normalizeUiText(raw?.organUnitName ?? raw?.organ_unit_name),
  organUnitCode: normalizeUiText(raw?.organUnitCode ?? raw?.organ_unit_code),
  organUnitHierarchy: String(raw?.organUnitHierarchy ?? raw?.organ_unit_hierarchy ?? "").trim(),
  kcehNumber: normalizeUiText(raw?.kcehNumber ?? raw?.kceh_number),
  employees: Array.isArray(raw?.employees)
    ? raw.employees.map((item) => normalizeOrganUnitDescendantEmployee(item))
    : []
});

const buildOrganizationStructureTree = (flatItems) => {
  if (!Array.isArray(flatItems) || flatItems.length === 0) {
    return [];
  }
  const rows = flatItems.map((item) => ({
    organUnitChildId: item.organUnitChildId,
    organUnitName: item.organUnitName?.trim() ? item.organUnitName : "-",
    organUnitCode: item.organUnitCode?.trim() ? item.organUnitCode : "-",
    kcehNumber: item.kcehNumber?.trim() ? item.kcehNumber : "-",
    organUnitHierarchy: item.organUnitHierarchy,
    employees: Array.isArray(item.employees) ? item.employees : [],
    children: []
  }));
  const byKey = new Map(rows.map((r) => [r.organUnitHierarchy, r]));
  const roots = [];
  for (const row of rows) {
    const parts = row.organUnitHierarchy.split(".").filter(Boolean);
  ...
  const sortRecursive = (nodes) => {
    nodes.sort((a, b) =>
      String(a.organUnitName).localeCompare(String(b.organUnitName), "ru", {
        sensitivity: "base",
        numeric: true
      })
    );
...
const flattenOrganizationStructureTreeForPanel = (nodes, collapsedIds, depth = 0, acc = []) => {
  ...
  for (const node of nodes) {
    ...
    const nodeId = String(rest.organUnitChildId ?? "").trim();
    const hierarchyKey = String(rest.organUnitHierarchy ?? "").trim();
    const collapseKey = nodeId || hierarchyKey;
    const isCollapsed = hasChildren && Boolean(collapseKey) && collapsed.has(collapseKey);
    acc.push({
      ...rest,
      depth,
      hasChildren,
    ...
    if (hasChildren && collapseKey && !collapsed.has(collapseKey)) {
      flattenOrganizationStructureTreeForPanel(kids, collapsed, depth + 1, acc);
    }
  }
  return acc;
};

/** Родитель строки структуры по organUnitHierarchy (сегменты из API) и id головной организации */
const resolveStructureRowParentOrganUnitId = (row, structureRootOrganUnitId, allRows) => {
  const rootId = String(structureRootOrganUnitId ?? "").trim();
  if (!row || !rootId) {
    return rootId;
  }
  const byHierarchy = new Map(
    (Array.isArray(allRows) ? allRows : []).map((r) => [String(r.organUnitHierarchy ?? "").trim(), r])
  );
  const parts = String(row.organUnitHierarchy ?? "").split(".").filter(Boolean);
  if (parts.length <= 1) {
    return rootId;
  }
  const parentKey = parts.slice(0, -1).join(".");
  const parent = byHierarchy.get(parentKey);
  return String(parent?.organUnitChildId ?? "").trim() || rootId;
};

/** true, если candidate — потомок ancestor в дереве структуры (по hierarchy), не сам ancestor */
const isStructureRowDescendantOf = (candidateRow, ancestorRow) => {
  const ah = String(ancestorRow?.organUnitHierarchy ?? "").trim();
  const ch = String(candidateRow?.organUnitHierarchy ?? "").trim();
  if (!ah || !ch || ch === ah) {
    return false;
  }
  return ch.startsWith(`${ah}.`);
};
...
const normalizeReportTemplateField = (field, index) => {
  ...
  const { fielOrderNumber: _omitLegacyFieldOrderKey, ...restSource } = source;
  const normalized = {
  ...
  /* Неизвестные атрибуты поля из report_info сохраняем в jsonb вместе с известными */
  return { ...restSource, ...normalized };
};
...
const buildLowercaseToCanonicalMap = (names) => {
  const map = new Map();
  names.forEach((name) => {
...
const reconcileReportTemplateFieldsWithSqlColumns = (
  ...
  const dataColumnLowerToCanonical = buildLowercaseToCanonicalMap(dataColumns);
  const linkColumnLowerToCanonical = buildLowercaseToCanonicalMap(linkColumns);
  const seenFieldNamesLower = new Set();
  ...
  normalized.forEach((field) => {
    ...
    const canonicalName = dataColumnLowerToCanonical.get(fieldName.toLowerCase());
    if (!canonicalName || seenFieldNamesLower.has(fieldName.toLowerCase())) {
      return;
    }
    seenFieldNamesLower.add(fieldName.toLowerCase());
    const nextField = { ...field, fieldName: canonicalName };
    const normalizedLink = String(nextField.fieldLink ?? "").trim();
    if (/^LINK_/i.test(normalizedLink)) {
      const linkCanonical = linkColumnLowerToCanonical.get(normalizedLink.toLowerCase());
      nextField.fieldLink = linkCanonical || "";
    }
  ...
  if (addMissing) {
    dataColumns.forEach((columnName) => {
      if (seenFieldNamesLower.has(columnName.toLowerCase())) {
        return;
      }
      seenFieldNamesLower.add(columnName.toLowerCase());
      nextFields.push(
...
function parseStoredOrganizationCardRelationColumnWidths() {
  if (typeof window === "undefined") {
    return { ...DEFAULT_ORGANIZATION_RELATION_COLUMN_WIDTHS };
  }
  const rawValue = window.localStorage.getItem(ORGANIZATION_CARD_RELATION_COLUMN_WIDTHS_STORAGE_KEY);
  if (!rawValue) {
    return { ...DEFAULT_ORGANIZATION_RELATION_COLUMN_WIDTHS };
  }
  try {
    ...
    const next = { ...DEFAULT_ORGANIZATION_RELATION_COLUMN_WIDTHS };
    for (const column of ORGANIZATION_RELATION_COLUMNS) {
      const rawWidth = Number(parsed[column.key]);
      if (Number.isFinite(rawWidth)) {
        next[column.key] = Math.max(MIN_COLUMN_WIDTH, Math.round(rawWidth));
      }
    }
    return next;
  } catch {
...
function parseStoredOrganizationCardRelationSortRules() {
  if (typeof window === "undefined") {
    return [];
  }
  const rawValue = window.localStorage.getItem(ORGANIZATION_CARD_RELATION_SORT_RULES_STORAGE_KEY);
  if (!rawValue) {
    return [];
  }
  try {
    ...
    const allowed = new Set(ORGANIZATION_RELATION_COLUMNS.map((column) => column.key));
    return parsed.filter((item) => {
      const field = snakeToCamel(String(item?.field ?? "").trim());
      const direction = String(item?.direction ?? "")
        .trim()
        .toUpperCase();
      return allowed.has(field) && ALLOWED_SORT_DIRECTIONS.has(direction);
    });
  } catch {
    return [];
  }
...
function parseStoredOrganizationCardRelationFilters() {
  if (typeof window === "undefined") {
    return { ...INITIAL_ORGANIZATION_RELATION_FILTERS };
  }
  const rawValue = window.localStorage.getItem(ORGANIZATION_CARD_RELATION_FILTERS_STORAGE_KEY);
  if (!rawValue) {
    return { ...INITIAL_ORGANIZATION_RELATION_FILTERS };
  }
  try {
...
const DEFAULT_ORGANIZATION_STRUCTURE_COL_FRACS = {
  name: 0.45,
  code: 0.28,
  kceh: 0.27
};

function parseStoredOrganizationStructureColumnFracs() {
  if (typeof window === "undefined") {
    return DEFAULT_ORGANIZATION_STRUCTURE_COL_FRACS;
  }

  const rawValue = window.localStorage.getItem(ORGANIZATION_STRUCTURE_COLUMN_FRACS_STORAGE_KEY);
  if (!rawValue) {
    return DEFAULT_ORGANIZATION_STRUCTURE_COL_FRACS;
  }
  ...
  try {
    ...
    const name = Number(parsed.name);
    const code = Number(parsed.code);
    const kceh = Number(parsed.kceh);
    if (!Number.isFinite(name) || !Number.isFinite(code) || !Number.isFinite(kceh)) {
      return DEFAULT_ORGANIZATION_STRUCTURE_COL_FRACS;
    }
    if (name <= 0 || code <= 0 || kceh <= 0) {
      return DEFAULT_ORGANIZATION_STRUCTURE_COL_FRACS;
    }
    const sum = name + code + kceh;
...
function sanitizeOrganizationStructurePanesSplit(frac) {
  const n = Number(frac);
...
function parseStoredOrganizationStructurePanesSplit() {
  if (typeof window === "undefined") {
    return 0.5;
  }
  try {
    const raw = window.localStorage.getItem(ORGANIZATION_STRUCTURE_PANES_SPLIT_STORAGE_KEY);
    return sanitizeOrganizationStructurePanesSplit(Number(raw));
  } catch {
    /* noop */
  }
  return 0.5;
...
function computeOrgStructureDragScrollIntent(wrapper, clientX, clientY) {
  if (!(wrapper instanceof HTMLElement) || !Number.isFinite(clientY) || !Number.isFinite(clientX)) {
...
function App() {
  ...
  const [isAdministrationClosing, setIsAdministrationClosing] = useState(false);
  const [isReferenceBooksSectionExpanded, setIsReferenceBooksSectionExpanded] = useState(true);
  ...
  const reportSettingsChunkPrefetchStartedRef = useRef(false);
  const [activePage, setActivePage] = useState(initialPageId);
  const [referenceBookDataUrlSuffix, setReferenceBookDataUrlSuffix] = useState(() =>
    getReferenceBookDataSuffixFromUrl()
  );
  const [referenceBookSettingsUrlSuffix, setReferenceBookSettingsUrlSuffix] = useState(() =>
    getReferenceBookSettingsSuffixFromUrl()
  );
  const [referenceBookDataToolbar, setReferenceBookDataToolbar] = useState({ addRecords: false });
  const [referenceBookDataHeaderTitle, setReferenceBookDataHeaderTitle] = useState("");
  ...
  const [organizationStructureItems, setOrganizationStructureItems] = useState([]);
  const [organizationStructureHeadEmployees, setOrganizationStructureHeadEmployees] = useState([]);
  const [organizationStructureLoading, setOrganizationStructureLoading] = useState(false);
  const [organizationStructureError, setOrganizationStructureError] = useState("");
  const [organizationStructureColFracs, setOrganizationStructureColFracs] = useState(
    parseStoredOrganizationStructureColumnFracs
  );
  const [organizationStructurePanesSplitFrac, setOrganizationStructurePanesSplitFrac] = useState(
    parseStoredOrganizationStructurePanesSplit
  );
  const [organizationStructureFluidWidth, setOrganizationStructureFluidWidth] = useState(360);
  const organizationStructureLayoutRef = useRef(null);
  const organizationStructureTableWrapperRef = useRef(null);
  const organizationStructureEmployeesTableWrapRef = useRef(null);
  const organizationStructureResizeRef = useRef(null);
  const organizationStructurePanesResizeRef = useRef(null);
  const structureDraggedOrganUnitIdRef = useRef("");
  const structureDraggedEmployeeOrganIdRef = useRef("");
  const structureDragAutoScrollStateRef = useRef({
    rafId: 0,
    intents: []
  });
  const [organizationStructureCollapsedIds, setOrganizationStructureCollapsedIds] = useState(
    () => new Set()
  );
  const [structureDraggingOrganUnitId, setStructureDraggingOrganUnitId] = useState("");
  const [structureDropTargetId, setStructureDropTargetId] = useState("");
  const [organizationStructureSelectedRowId, setOrganizationStructureSelectedRowId] = useState("");
  /** Синхронизация фильтра «Подразделение» в таблице сотрудников с выбранной строкой структуры */
  const [structureEmployeesSubdivisionFilterSync, setStructureEmployeesSubdivisionFilterSync] = useState({
    tick: 0,
    value: ""
  });
  ...
  const selectReportCardTab = useCallback((tab) => {
    if (tab === REPORT_CARD_TABS.TEMPLATE) {
      prefetchReportTemplateJsonEditorPanel();
    }
    if (tab === REPORT_CARD_TABS.SQL) {
      prefetchPrismSqlHighlighter();
    }
    startTransition(() => {
      setActiveReportCardTab(tab);
    });
  }, []);
  const [reportSqlViewMode, setReportSqlViewMode] = useState(REPORT_SQL_VIEW_MODES.EDITOR);
  /** Загружается лениво (отдельный чанк), пока null — подсветка SQL как экранированный текст. */
  const [reportSqlPrism, setReportSqlPrism] = useState(null);
  ...
  /** Последний employeeId, для которого успешно завершилась загрузка связей (для фона без «мигания» loading) */
  const lastEmployeeRelationsLoadedForIdRef = useRef(null);
  const employeeRelationsRef = useRef(employeeRelations);
  employeeRelationsRef.current = employeeRelations;
  const [organizationCardRelations, setOrganizationCardRelations] = useState([]);
  const [organizationCardRelationsLoading, setOrganizationCardRelationsLoading] = useState(false);
  const [organizationCardRelationsError, setOrganizationCardRelationsError] = useState("");
  const [organizationCardRelationsSortRules, setOrganizationCardRelationsSortRules] = useState(() =>
    parseStoredOrganizationCardRelationSortRules()
  );
  const [organizationCardRelationsFilters, setOrganizationCardRelationsFilters] = useState(() =>
    parseStoredOrganizationCardRelationFilters()
  );
  const [organizationCardRelationColumnWidths, setOrganizationCardRelationColumnWidths] = useState(() =>
    parseStoredOrganizationCardRelationColumnWidths()
  );
  const [selectedOrganizationCardRelationKey, setSelectedOrganizationCardRelationKey] = useState("");
  const [selectedEmployeeCardRelationKey, setSelectedEmployeeCardRelationKey] = useState("");
  ...
  const [pendingOrganizationSubdivisionDelete, setPendingOrganizationSubdivisionDelete] = useState(null);
  const [organizationStructureEditor, setOrganizationStructureEditor] = useState(null);
  const [organizationStructureEditorSaving, setOrganizationStructureEditorSaving] = useState(false);
  ...
  const latestEmployeePositionFormRef = useRef(INITIAL_NEW_POSITION_FORM);
  latestEmployeePositionFormRef.current = newEmployeePositionForm;
  ...
  const organizationCardRelationsTabContentRef = useRef(null);
  const organizationCardRelationsTableWrapperRef = useRef(null);
  ...
  /** Чтобы не затирать reportSqlDraft при refresh списка во время редактирования SQL */
  const reportSqlCardTemplateIdRef = useRef("");
  ...
  const reportTemplateSettingsDraftFieldsRef = useRef([]);
  ...
  const administrationCloseTimeoutRef = useRef(0);
  ...
  const [settingsPanelBounds, setSettingsPanelBounds] = useState({ top: 80, bottom: 24, right: 24 });
  ...
  const isReferenceBookDataPage = activePage === PAGE_IDS.REFERENCE_BOOK_DATA;
  const currentPageTitle =
    activePage === PAGE_IDS.REFERENCE_BOOK_DATA
      ? String(referenceBookDataHeaderTitle ?? "").trim() || PAGE_TITLES[PAGE_IDS.REFERENCE_BOOK_DATA]
      : PAGE_TITLES[activePage] ?? PAGE_TITLES[PAGE_IDS.EMPLOYEES];
  ...
  const isReferenceBooksPage = activePage === PAGE_IDS.REFERENCE_BOOKS;
  ...
  const organizationStructureTree = useMemo(
    () => buildOrganizationStructureTree(organizationStructureItems),
    [organizationStructureItems]
  );
  const organizationStructureFlatRows = useMemo(
    () =>
      flattenOrganizationStructureTreeForPanel(organizationStructureTree, organizationStructureCollapsedIds),
    [organizationStructureTree, organizationStructureCollapsedIds]
  );
  const organizationStructureRootDisplayName = useMemo(() => {
    const source = selectedOrganization;
    if (!source) {
      return "";
    }
    const raw =
      ...
      source.organ_name ??
      "";
    return normalizeUiText(raw);
  }, [selectedOrganization]);
  const organizationStructureParentSelectOptions = useMemo(() => {
    const rootId = String(
    ...
    const rootLabel = organizationStructureRootDisplayName || "Головная организация";
    if (!rootId) {
      return [];
    }
    const opts = [{ value: rootId, label: rootLabel }];
    const sorted = [...organizationStructureItems].sort((a, b) =>
      String(a.organUnitHierarchy ?? "").localeCompare(String(b.organUnitHierarchy ?? ""), "ru", {
        sensitivity: "base",
        numeric: true
      })
    );
    for (const row of sorted) {
      const depth = String(row.organUnitHierarchy ?? "").split(".").filter(Boolean).length;
      const indent = depth > 1 ? "— ".repeat(depth - 1) : "";
      opts.push({
        value: row.organUnitChildId,
        label: `${indent}${row.organUnitName || "-"}`
      });
    }
    return opts;
  }, [
    organizationStructureItems,
    organizationStructureRootDisplayName,
    selectedOrganization,
    selectedOrganizationId
  ]);
  const organizationStructureEditorParentSelectOptions = useMemo(() => {
    if (!organizationStructureEditor || organizationStructureEditor.mode !== "edit") {
      return organizationStructureParentSelectOptions;
    }
    const editId = String(organizationStructureEditor.organUnitChildId ?? "").trim();
    const editRow = organizationStructureItems.find((r) => r.organUnitChildId === editId);
    if (!editRow) {
      return organizationStructureParentSelectOptions;
    }
    return organizationStructureParentSelectOptions.filter((opt) => {
      if (opt.value === editId) {
        return false;
      }
      const row = organizationStructureItems.find((r) => r.organUnitChildId === opt.value);
      if (!row) {
        return true;
      }
      return !isStructureRowDescendantOf(row, editRow);
    });
  }, [
    organizationStructureEditor,
    organizationStructureItems,
    organizationStructureParentSelectOptions
  ]);
  const organizationStructureColWidthsPx = useMemo(() => {
    const fw = Math.max(
      ORGANIZATION_STRUCTURE_MIN_DATA_COL_PX * 3,
      organizationStructureFluidWidth
    );
    const { name: fn, code: fc } = organizationStructureColFracs;
    const minPx = ORGANIZATION_STRUCTURE_MIN_DATA_COL_PX;
    let namePx = Math.max(minPx, Math.round(fw * fn));
    let codePx = Math.max(minPx, Math.round(fw * fc));
    let kcehPx = fw - namePx - codePx;
    if (kcehPx < minPx) {
      let need = minPx - kcehPx;
      const fromName = Math.min(need, namePx - minPx);
      namePx -= fromName;
      need -= fromName;
      kcehPx = fw - namePx - codePx;
      if (kcehPx < minPx && need > 0) {
        const fromCode = Math.min(need, codePx - minPx);
        codePx -= fromCode;
        kcehPx = fw - namePx - codePx;
      }
    }
    return {
      namePx,
      codePx,
      kcehPx: Math.max(minPx, kcehPx)
    };
  }, [organizationStructureFluidWidth, organizationStructureColFracs]);
  ...
  const isOrganizationCardRelationsTab =
    isOrganizationCardVisible && activeOrganizationCardTab === ORGANIZATION_CARD_TABS.RELATIONS;
  ...
  const isEmployeeRelatedHeaderContext =
    isEmployeeOrganizationPage || isEmployeeRelationsPage || isEmployeeCardPanelOpen || isOrganizationCardPanelOpen;
  const isReportHeaderContext = isReportSettingsPage || isReportCardPanelOpen;
  const isEmployeesHeaderActionsVisible = isEmployeesPage || isEmployeeCardPanelOpen;
  const isOrganizationsHeaderActionsVisible = isOrganizationsPage || isOrganizationCardPanelOpen;
  const isRelationsHeaderActionsVisible = isEmployeeRelationsPage;
  const shouldRenderHeaderActions =
    isReportHeaderContext ||
    isEmployeesHeaderActionsVisible ||
    isOrganizationsHeaderActionsVisible ||
    isRelationsHeaderActionsVisible ||
    isReferenceBooksPage ||
    isReferenceBookDataPage;
  const headerActionsMode = isReportHeaderContext
    ? "report"
    : isEmployeesHeaderActionsVisible
      ? "employees"
      : isRelationsHeaderActionsVisible
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void loadPrismSqlModule().then((Prism) => {
      if (!cancelled) {
        setReportSqlPrism((prev) => prev ?? Prism);
      }
    });
    return () => {
      cancelled = true;
    };
  }, [activeReportCardTab, isReportCardVisible, isReportSettingsPage]);
  ...
  const highlightedReportSql = useMemo(() => {
    ...
    if (!reportSqlPrism) {
      return escapeHtmlForSqlHighlight(normalizedSql);
    }
    return reportSqlPrism.highlight(normalizedSql, reportSqlPrism.languages.sql, "sql");
  }, [reportSqlPrism, reportSqlSourceText]);
  ...
  // Не завязывать на activeReportCardTab: при смене вкладки карточки отчёта не появляются новые [title],
  // а полный querySelectorAll по документу на каждый клик давал задержку до секунд.
  useEffect(() => {
  ...
  useEffect(() => {
    const getTooltipTarget = (target) => {
      ...
      if (!String(el.getAttribute("data-tooltip") ?? "").trim()) {
        return null;
      }
      return el;
    };
    ...
    const handleMouseOver = (event) => {
      // Не вешаем React-тултип на кнопки карточки отчёта: setState на каждый mouseover
      // блокирует отрисовку :hover (задержка ~1 с). Подсветка кнопок — чистый CSS.
      if (
        event.target instanceof Element &&
        event.target.closest(".report-template-card-panel")
      ) {
        return;
      }
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        ORGANIZATION_STRUCTURE_COLUMN_FRACS_STORAGE_KEY,
        JSON.stringify(organizationStructureColFracs)
      );
    } catch {
      /* localStorage недоступен или переполнен */
    }
  }, [organizationStructureColFracs]);

  useEffect(() => {
    try {
      window.localStorage.setItem(
        ORGANIZATION_STRUCTURE_PANES_SPLIT_STORAGE_KEY,
        String(organizationStructurePanesSplitFrac)
      );
    } catch {
      /* noop */
    }
  }, [organizationStructurePanesSplitFrac]);
  ...
  useEffect(() => {
    window.localStorage.setItem(
      ORGANIZATION_CARD_RELATION_COLUMN_WIDTHS_STORAGE_KEY,
      JSON.stringify(organizationCardRelationColumnWidths)
    );
  }, [organizationCardRelationColumnWidths]);

  useEffect(() => {
    window.localStorage.setItem(
      ORGANIZATION_CARD_RELATION_SORT_RULES_STORAGE_KEY,
      JSON.stringify(organizationCardRelationsSortRules)
    );
  }, [organizationCardRelationsSortRules]);

  useEffect(() => {
    window.localStorage.setItem(
      ORGANIZATION_CARD_RELATION_FILTERS_STORAGE_KEY,
      JSON.stringify(organizationCardRelationsFilters)
    );
  }, [organizationCardRelationsFilters]);
  ...
  useEffect(() => {
    if (!isReportCardVisible) {
      ...
      reportSqlCardTemplateIdRef.current = "";
    ...
    const templateId = String(selectedReportTemplateId ?? "").trim();
    const templateChanged = reportSqlCardTemplateIdRef.current !== templateId;
    if (templateChanged) {
      reportSqlCardTemplateIdRef.current = templateId;
      setIsReportSqlEditMode(false);
    ...
    setReportSqlEditorScrollTop(0);
  }, [isReportCardVisible, reportSqlText, selectedReportTemplateId, isReportSqlEditMode]);
  ...
  const fetchReportPreviewData = async (reportTemplateId) => {
    const xlsxPromise = loadXlsxModule();
    let XLSX;
  ...
  useEffect(() => {
    ...
    setSelectedEmployeeCardRelationKey("");
    lastEmployeeRelationsLoadedForIdRef.current = null;
  ...
  useEffect(() => {
    ...
    const fetchRelations = async () => {
      const relationsEmployeeId = String(selectedEmployeeIdForRelations ?? "").trim();
      const alreadyLoadedForSame =
        lastEmployeeRelationsLoadedForIdRef.current === relationsEmployeeId &&
        employeeRelationsRef.current.length > 0;
      if (!alreadyLoadedForSame) {
  ...
  useEffect(() => {
    ...
    setOrganizationCardRelations([]);
    setOrganizationCardRelationsError("");
    setIsAddingEmployeeRelation(false);
    setEditingEmployeeRelationId("");
    setActiveNewRelationCombo(null);
    setRelationComboMenuLayouts({});
    ...
    setSalesOrganizationOptions([]);
    setProductGroupOptions([]);
    setSelectedOrganizationCardRelationKey("");
  }, [selectedOrganizationId, isOrganizationsPage]);

  useEffect(() => {
    if (!isOrganizationCardRelationsTab || !String(selectedOrganizationId ?? "").trim()) {
      return;
    }

    const fetchOrgRelations = async () => {
      setOrganizationCardRelationsLoading(true);
      setOrganizationCardRelationsError("");

      const requestBody = {
        organUnitId: String(selectedOrganizationId ?? "").trim(),
        sorts:
          organizationCardRelationsSortRules.length > 0
            ? organizationCardRelationsSortRules
            : [{ field: "employeeName", direction: "ASC" }]
      };

      for (const [key, value] of Object.entries(organizationCardRelationsFilters)) {
        const normalized = String(value ?? "").trim();
        if (!normalized) {
          continue;
        }
      ...
      try {
        const response = await fetch(`${ADMIN_API_BASE_URL}/relations`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
        ...
        if (!response.ok) {
          setOrganizationCardRelations([]);
          setOrganizationCardRelationsError(data.error ?? "Не удалось получить связи организации");
          return;
        }

        setOrganizationCardRelations(Array.isArray(data.items) ? data.items : []);
      } catch {
        setOrganizationCardRelations([]);
        setOrganizationCardRelationsError("Не удалось получить связи организации");
      } finally {
        setOrganizationCardRelationsLoading(false);
      }
    };
    ...
    API_BASE_URL,
    isOrganizationCardRelationsTab,
    organizationCardRelationsFilters,
    organizationCardRelationsSortRules,
    selectedOrganizationId
  ]);
  ...
  const calculateSettingsPanelBounds = useCallback(() => {
    ...
    const mainRect = mainPanelElement?.getBoundingClientRect();
    if (!mainRect) {
      return null;
    }

    const listLayoutRect =
      mainPanelElement?.querySelector(".list-content-layout")?.getBoundingClientRect() ?? mainRect;
  ...
  const updateSettingsPanelBounds = useCallback(() => {
    const nextBounds = calculateSettingsPanelBounds();
    if (!nextBounds) {
      return;
    }

    setSettingsPanelBounds(nextBounds);
  ...
  useEffect(() => {
    ...
    window.addEventListener("resize", updateSettingsPanelBounds);
    return () => {
      window.removeEventListener("resize", updateSettingsPanelBounds);
    };
  }, [isColumnSettingsOpen, updateSettingsPanelBounds]);
  ...
  useEffect(() => {
    return () => {
      if (administrationCloseTimeoutRef.current) {
        window.clearTimeout(administrationCloseTimeoutRef.current);
        administrationCloseTimeoutRef.current = 0;
      }
    };
  }, []);

  useEffect(() => {
    if (!isReportBuilderExpanded || reportSettingsChunkPrefetchStartedRef.current) {
      return;
    }
    reportSettingsChunkPrefetchStartedRef.current = true;
    prefetchReportSettingsPage();
  ...
  const listTableLayout = useMemo(() => {
    ...
    const visibleColumnsOrdered = orderedColumns.filter(
      (column) => settingsMap.get(column.key)?.visible !== false
    );
  ...
  }, [columnSettings, columnWidths, tableColumns, defaultColumnWidthsForPage, isEmployeeRelationsPage]);
  ...
  const relationsListVirtualizationEligible = useMemo(
    () =>
      isEmployeeRelationsPage &&
      !isAddingEmployeeRelation &&
      !String(editingEmployeeRelationId ?? "").trim(),
    [isEmployeeRelationsPage, isAddingEmployeeRelation, editingEmployeeRelationId]
  );

  const listVirtualizationActive = useMemo(() => {
    const eligible =
      isOrganizationsPage ||
      isReportSettingsPage ||
      isEmployeesPage ||
      relationsListVirtualizationEligible;
    return (
      eligible &&
      !employeesLoading &&
      !employeesError &&
      displayedEmployees.length > 0 &&
      (pageSize >= LIST_VIRTUALIZATION_PAGE_SIZE_THRESHOLD ||
        displayedEmployees.length >= LIST_VIRTUALIZATION_ROW_COUNT_THRESHOLD)
    );
  }, [
    isOrganizationsPage,
    isReportSettingsPage,
  ...
  const [mainListTheadScrollMargin, setMainListTheadScrollMargin] = useState(96);
  useLayoutEffect(() => {
    if (!listVirtualizationActive || !tableWrapperRef.current) {
      return;
    }
    const thead = tableWrapperRef.current.querySelector("table thead");
    if (!thead) {
      return;
    }
    const next = Math.ceil(thead.getBoundingClientRect().height);
    setMainListTheadScrollMargin((prev) => (prev !== next ? next : prev));
  }, [
  ...
  const listVirtualizer = useVirtualizer({
    count: listVirtualizationActive ? displayedEmployees.length : 0,
    getScrollElement: () => tableWrapperRef.current,
    estimateSize: () => LIST_ROW_ESTIMATE_PX,
    overscan: 12,
    scrollMargin: mainListTheadScrollMargin
  });
  ...
  const requestOrganizationCardRelationsExport = async () => {
    const organUnitId = String(selectedOrganizationId ?? "").trim();
    if (!organUnitId) {
    ...
    const requestBody = {
      ...
      sorts:
        organizationCardRelationsSortRules.length > 0
          ? organizationCardRelationsSortRules
          : [{ field: "employeeName", direction: "ASC" }],
      columns: ORGANIZATION_RELATION_COLUMNS.map((column) => ({
        key: column.key,
        title: column.title
      }))
    };
    ...
    const response = await fetch(`${ADMIN_API_BASE_URL}/relations/export`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    if (!response.ok) {
      let errorMessage = "Не удалось выгрузить связи организации в Excel";
      try {
        const errorPayload = await response.json();
        if (typeof errorPayload?.error === "string" && errorPayload.error.trim()) {
          errorMessage = errorPayload.error;
    ...
    const fileName = parseFileNameFromDisposition(
      response.headers.get("content-disposition"),
      "organization-relations-export.xlsx"
    );
    return { blob, fileName };
  };
  ...
  const closeAdministrationPanel = useCallback(() => {
    if (!isAdministrationOpen && !isAdministrationClosing) {
      return;
    }
    setIsAdministrationOpen(false);
    setIsAdministrationClosing(true);
    if (administrationCloseTimeoutRef.current) {
      window.clearTimeout(administrationCloseTimeoutRef.current);
    }
    administrationCloseTimeoutRef.current = window.setTimeout(() => {
      setIsAdministrationClosing(false);
      administrationCloseTimeoutRef.current = 0;
    }, 220);
  }, [isAdministrationClosing, isAdministrationOpen]);

  const handleAdministrationClick = () => {
    if (isAdministrationOpen) {
      closeAdministrationPanel();
      return;
    }
  ...
  const handlePageSelect = (pageId) => {
    setReferenceBookDataUrlSuffix("");
    setReferenceBookSettingsUrlSuffix("");
    setReferenceBookDataToolbar({ addRecords: false });
    setReferenceBookDataHeaderTitle("");
    if (isEmployeesPage) {
      window.localStorage.setItem(SORT_RULES_STORAGE_KEY, JSON.stringify(sortRules));
    } else if (isOrganizationsPage) {
      window.localStorage.setItem(ORGANIZATIONS_SORT_RULES_STORAGE_KEY, JSON.stringify(sortRules));
    ...
    if (pageId === PAGE_IDS.EMPLOYEES) {
      ...
      setActivePrintFormsTab(PRINT_FORMS_TABS.LIST);
      setPrintFormsError("");
    } else if (pageId === PAGE_IDS.REFERENCE_BOOKS) {
      setSortRules(parseStoredSortRules());
      setFilters(parseStoredFilters());
      setDebouncedEmployeeFilters(parseStoredFilters());
      setDebouncedOrganizationFilters(parseStoredOrganizationFilters());
  ...
  const handleReferenceBookSettingsNavigate = useCallback((suffix) => {
    const s = String(suffix ?? "").trim();
    setReferenceBookSettingsUrlSuffix(s);
    if (typeof window === "undefined") {
      return;
    }
    const nextPath = s
      ? `/reference-books/settings/${encodeURIComponent(s)}`
      : PAGE_PATHNAMES[PAGE_IDS.REFERENCE_BOOKS];
    const current = normalizePathname(window.location.pathname);
    if (normalizePathname(nextPath) === current) {
      return;
    }
  ...
  const handleOpenReferenceBookData = useCallback((suffix) => {
    const s = String(suffix ?? "").trim();
    if (!s) {
      return;
    }
    ...
    window.history.pushState(
      window.history.state,
      "",
      `/reference-books/${encodeURIComponent(s)}`
    );
    setReferenceBookDataToolbar({ addRecords: false });
    setReferenceBookDataHeaderTitle("");
    setReferenceBookDataUrlSuffix(s);
    setActivePage(PAGE_IDS.REFERENCE_BOOK_DATA);
  }, []);

  const handleBackFromReferenceBookData = useCallback(() => {
    window.history.pushState(window.history.state, "", PAGE_PATHNAMES[PAGE_IDS.REFERENCE_BOOKS]);
    setReferenceBookDataUrlSuffix("");
    setReferenceBookSettingsUrlSuffix("");
    setReferenceBookDataToolbar({ addRecords: false });
    setReferenceBookDataHeaderTitle("");
    setActivePage(PAGE_IDS.REFERENCE_BOOKS);
  }, []);

  useEffect(() => {
    const onPop = () => {
      const pid = getPageIdFromUrl();
      setActivePage(pid);
      setReferenceBookDataUrlSuffix(getReferenceBookDataSuffixFromUrl());
      setReferenceBookSettingsUrlSuffix(getReferenceBookSettingsSuffixFromUrl());
      if (pid !== PAGE_IDS.REFERENCE_BOOK_DATA) {
        setReferenceBookDataToolbar({ addRecords: false });
        setReferenceBookDataHeaderTitle("");
      }
    };
  ...
  const handleOrganizationRowMouseDown = useCallback((event) => {
    if (event.button !== 0) {
      return;
    }
  ...
  const handleReportRowClickRef = useRef(handleReportRowClick);
  handleReportRowClickRef.current = handleReportRowClick;
  const stableHandleReportRowClick = useCallback((rowIndex) => {
    handleReportRowClickRef.current(rowIndex);
  }, []);

  const handleOrganizationRowClickRef = useRef(handleOrganizationRowClick);
  handleOrganizationRowClickRef.current = handleOrganizationRowClick;
  const stableHandleOrganizationRowClick = useCallback((rowIndex) => {
    handleOrganizationRowClickRef.current(rowIndex);
  }, []);

  const handleEmployeeRowClickRef = useRef(handleEmployeeRowClick);
  handleEmployeeRowClickRef.current = handleEmployeeRowClick;
  const stableHandleEmployeeRowClick = useCallback((rowIndex) => {
    handleEmployeeRowClickRef.current(rowIndex);
  }, []);
  ...
  const handleCloseOrganizationCardPanel = () => {
    setIsOrganizationCardPanelOpen(false);
    setActiveOrganizationCardTab(ORGANIZATION_CARD_TABS.MAIN);
    setOrganizationStructureItems([]);
    setOrganizationStructureHeadEmployees([]);
    setOrganizationStructureError("");
  ...
  const checkReportSqlSyntax = async (
    ...
    try {
      ...
      let response = await fetch(REPORT_TEMPLATE_SQL_VALIDATE_API_URL, {
        ...
        body: JSON.stringify(sqlValidateBody)
      });
      if (response.status === 404) {
  ...
  reportTemplateSettingsDraftFieldsRef.current = Array.isArray(reportTemplateSettingsDraft.fields)
  ...
  const handleReportTemplateFieldDragStart = useCallback(
    (sourceIndex, fieldName, event) => {
      ...
      const fields = reportTemplateSettingsDraftFieldsRef.current;
      const sourceField = Array.isArray(fields) ? fields[sourceIndex] : null;
  ...
  const handleReportTemplateFieldDragOver = useCallback(
    (targetIndex, event) => {
      ...
      const fields = reportTemplateSettingsDraftFieldsRef.current;
      const sourceField = Array.isArray(fields)
        ? fields[reportTemplateFieldsDragSourceRef.current]
        : null;
      const targetField = Array.isArray(fields) ? fields[targetIndex] : null;
  ...
  const handleToggleReportTemplateJsonEdit = () => {
    ...
    if (isReportTemplateJsonEditMode) {
      const latest = reportTemplateJsonEditorPanelRef.current?.getValue?.();
      if (typeof latest === "string") {
        setReportTemplateJsonDraft(latest);
      }
    }
    setIsReportTemplateJsonEditMode((prev) => !prev);
  };
  ...
  const handleChangeReportTemplateSettingsFieldRow = useCallback((index, fieldName, value) => {
    setReportTemplateSettingsDraft((prev) => {
  ...
  const loadReportTemplateSqlColumns = useCallback(
    async ({ showErrorToast = true, sqlQueryForRefresh } = {}) => {
      const reportTemplateId = String(selectedReportTemplateId ?? "").trim();
      if (!reportTemplateId) {
        return null;
      }
      try {
        const executionPayload = buildReportExecutionPayload(reportTemplateId);
        const payload =
          sqlQueryForRefresh !== undefined
            ? { ...executionPayload, sqlQuery: sqlQueryForRefresh }
            : executionPayload;
        const columns = await loadSqlColumnsForReportTemplate(reportTemplateId, payload);
  ...
  const handleRefreshReportTemplateFieldsFromSql = async () => {
    ...
    const allColumns = await loadReportTemplateSqlColumns({
      showErrorToast: true,
      sqlQueryForRefresh: String(reportSqlDraft ?? "")
    });
    if (!allColumns) {
      return;
    }
  ...
  const handleSaveReportTemplateJson = async () => {
    ...
    let parsedReportInfo;
    const jsonText =
      reportTemplateJsonEditorPanelRef.current?.getValue?.() ?? reportTemplateJsonDraft;
    try {
      parsedReportInfo = JSON.parse(String(jsonText ?? "{}"));
    } catch (error) {
      const syntaxErrorMessage = String(error?.message ?? "").trim();
      showSystemErrorToast(
        syntaxErrorMessage
  ...
  const handleEditOrSaveReportSql = async () => {
    ...
    /* Актуальный текст из textarea: state иногда отстаёт от DOM (обновления списка, вкладки). */
    const liveSql =
      reportSqlEditorRef.current && typeof reportSqlEditorRef.current.value === "string"
        ? String(reportSqlEditorRef.current.value)
        : String(reportSqlDraft ?? "");
    setReportSqlDraft(liveSql);
    const normalizedSql = liveSql.trim();
    if (normalizedSql) {
    ...
    try {
      let response = await fetch(`${ADMIN_API_BASE_URL}/report-template/${reportTemplateId}/sql`, {
        ...
        body: JSON.stringify(sqlPatchPayload)
      });
      if (response.status === 404) {
  ...
  const fetchOrganizationStructure = useCallback(async (organUnitId) => {
    const id = String(organUnitId ?? "").trim();
    if (!id) {
      setOrganizationStructureItems([]);
      setOrganizationStructureHeadEmployees([]);
      setOrganizationStructureError("");
    ...
    setOrganizationStructureError("");
    try {
      const response = await fetch(ORGANIZATION_DESCENDANTS_API_PATH(encodeURIComponent(id)));
      const data = await response.json();
      if (!response.ok) {
        setOrganizationStructureItems([]);
        setOrganizationStructureHeadEmployees([]);
        setOrganizationStructureError(
          String(data?.error ?? data?.message ?? "").trim() || "Не удалось загрузить структуру"
        );
        return;
      }
      const list = Array.isArray(data?.items) ? data.items : [];
      setOrganizationStructureItems(list.map((raw) => normalizeOrganStructureItem(raw)));
      const headList = Array.isArray(data?.headOrganEmployees) ? data.headOrganEmployees : [];
      setOrganizationStructureHeadEmployees(headList.map((raw) => normalizeOrganUnitDescendantEmployee(raw)));
    } catch {
      setOrganizationStructureItems([]);
      setOrganizationStructureHeadEmployees([]);
      setOrganizationStructureError("Не удалось загрузить структуру");
    } finally {
      setOrganizationStructureLoading(false);
    }
  }, []);

  const handleRefreshOrganizationStructure = useCallback(() => {
    const organizationId = String(
      selectedOrganization?.organUnitId ??
        selectedOrganization?.organ_unit_id ??
        selectedOrganization?.id ??
    ...
    return fetchOrganizationStructure(organizationId);
  }, [fetchOrganizationStructure, selectedOrganization, selectedOrganizationId]);

  const upsertOrganizationStructureEmployeeLink = useCallback(
    (payload) => {
      const rootId = String(
        selectedOrganization?.organUnitId ??
          selectedOrganization?.organ_unit_id ??
      ...
      const organUnitChildId = String(payload?.organUnitChildId ?? "").trim();
      if (!rootId || !organUnitChildId) {
        return;
      }
      const normalized = normalizeOrganUnitDescendantEmployee({
        employeeOrganId: payload?.employeeOrganId,
        employeeId: payload?.employeeId,
        employeeParentId: payload?.employeeParentId ?? "",
      ...
      const linkId = String(normalized.employeeOrganId ?? "").trim();
      if (!linkId || !normalized.employeeId) {
        return;
      }
      setOrganizationStructureHeadEmployees((prev) => {
        const without = prev.filter((r) => String(r.employeeOrganId) !== linkId);
        if (organUnitChildId === rootId) {
          return [...without, normalized];
        }
        return without;
      });
      setOrganizationStructureItems((prev) =>
        prev.map((item) => {
          const employees = item.employees.filter((e) => String(e.employeeOrganId) !== linkId);
          if (organUnitChildId !== rootId && String(item.organUnitChildId) === organUnitChildId) {
            return { ...item, employees: [...employees, normalized] };
          }
  ...
  const resolveSelectedStructureRootOrganUnitId = useCallback(() => {
    return String(
      selectedOrganization?.organUnitId ??
        selectedOrganization?.organ_unit_id ??
        selectedOrganization?.id ??
  ...
  const toggleOrganizationStructureNodeCollapse = useCallback((organUnitChildId) => {
    const id = String(organUnitChildId ?? "").trim();
    if (!id) {
      return;
    }
    setOrganizationStructureCollapsedIds((prev) => {
  ...
  const handleOrganizationStructureRowSelect = useCallback(
    (organUnitId) => {
      const id = String(organUnitId ?? "").trim();
      if (!id) {
        return;
      }
      setOrganizationStructureSelectedRowId(id);
      const rootId = String(
        selectedOrganization?.organUnitId ??
          selectedOrganization?.organ_unit_id ??
          selectedOrganization?.id ??
      ...
      if (id === rootId) {
        const raw =
          ...
          selectedOrganization?.organName ??
          selectedOrganization?.organ_name ??
          "";
        subdivisionName = normalizeUiText(raw);
      } else {
        const item = organizationStructureItems.find(
          (r) => String(r.organUnitChildId ?? "").trim() === id
        );
        subdivisionName = item ? normalizeUiText(String(item.organUnitName ?? "").trim()) : "";
      }
      setStructureEmployeesSubdivisionFilterSync((prev) => ({
        tick: prev.tick + 1,
        value: subdivisionName
      }));
    },
    [organizationStructureItems, selectedOrganization, selectedOrganizationId]
  );

  const stopOrganizationStructureDragAutoScroll = useCallback(() => {
    const state = structureDragAutoScrollStateRef.current;
    if (state.rafId) {
      cancelAnimationFrame(state.rafId);
    }
    state.rafId = 0;
  ...
  const setEmployeeStructureDnDActive = useCallback((employeeOrganId) => {
    structureDraggedEmployeeOrganIdRef.current = employeeOrganId
      ? String(employeeOrganId ?? "").trim()
      : "";
  }, []);

  const updateOrganizationStructureDragAutoScroll = useCallback((clientX, clientY) => {
    if (!Number.isFinite(clientY) || !Number.isFinite(clientX)) {
      return;
    }
    const wrappers = [
      organizationStructureTableWrapperRef.current,
      organizationStructureEmployeesTableWrapRef.current
    ...
    for (const w of wrappers) {
      const { direction, speed } = computeOrgStructureDragScrollIntent(w, clientX, clientY);
      if (direction && speed > 0) {
        intents.push({ el: w, direction, speed });
      }
    }
    const state = structureDragAutoScrollStateRef.current;
    ...
    let lastTimestamp = 0;
    const tick = () => {
      const activeState = structureDragAutoScrollStateRef.current;
      if (!activeState.rafId) {
        return;
      }
      const items = activeState.intents;
      if (!items.length) {
        activeState.rafId = 0;
        return;
      }
      ...
      for (const item of items) {
        if (item.el instanceof HTMLElement) {
          item.el.scrollTop += item.direction * (item.speed * deltaMs) / 1000;
        }
      }
      activeState.rafId = requestAnimationFrame(tick);
    };
  ...
  const findOrganizationStructureEmployeeLinkByOrganId = useCallback(
    (employeeOrganId) => {
      const id = String(employeeOrganId ?? "").trim();
      if (!id) {
        return null;
      }
      const rootId = resolveSelectedStructureRootOrganUnitId();
      if (!rootId) {
        return null;
      }
      for (const e of organizationStructureHeadEmployees) {
        if (String(e?.employeeOrganId ?? "").trim() === id) {
          return {
            employeeOrganId: id,
            employeeId: String(e?.employeeId ?? "").trim(),
            employeePositionId: String(e?.employeePositionId ?? "").trim(),
            employeeParentId: String(e?.employeeParentId ?? "").trim(),
            organUnitChildId: rootId
          };
        }
      }
      for (const item of organizationStructureItems) {
        const ouId = String(item?.organUnitChildId ?? "").trim();
        for (const row of Array.isArray(item?.employees) ? item.employees : []) {
          if (String(row?.employeeOrganId ?? "").trim() === id) {
            return {
              employeeOrganId: id,
              employeeId: String(row?.employeeId ?? "").trim(),
              employeePositionId: String(row?.employeePositionId ?? "").trim(),
              employeeParentId: String(row?.employeeParentId ?? "").trim(),
    ...
    [
      organizationStructureHeadEmployees,
      organizationStructureItems,
      resolveSelectedStructureRootOrganUnitId
    ]
  );

  const handleOrganizationStructureRowDragOver = useCallback(
    (dropTargetKey, event) => {
      event.preventDefault();
      event.dataTransfer.dropEffect = "move";
      const pointerX = Number(event?.clientX ?? event?.nativeEvent?.clientX);
      const pointerY = Number(event?.clientY ?? event?.nativeEvent?.clientY);
      const draggedOuId = String(structureDraggedOrganUnitIdRef.current ?? "").trim();
      const draggedEmpOrgId = String(structureDraggedEmployeeOrganIdRef.current ?? "").trim();
      if (
        (draggedOuId || draggedEmpOrgId) &&
        Number.isFinite(pointerX) &&
        Number.isFinite(pointerY)
      ) {
        updateOrganizationStructureDragAutoScroll(pointerX, pointerY);
      }
      const rootId = resolveSelectedStructureRootOrganUnitId();
      if (!rootId) {
        return;
      }
      const effectiveTargetOuId =
        dropTargetKey === ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY
          ? rootId
          : String(dropTargetKey ?? "").trim();
      if (!effectiveTargetOuId) {
        return;
      }

      if (draggedEmpOrgId) {
        const link = findOrganizationStructureEmployeeLinkByOrganId(draggedEmpOrgId);
        if (link && String(link.organUnitChildId) === effectiveTargetOuId) {
          return;
        }
        const empTargetRow =
          dropTargetKey === ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY
            ? null
            : organizationStructureItems.find((r) => r.organUnitChildId === effectiveTargetOuId);
        if (dropTargetKey !== ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY && !empTargetRow) {
          return;
        }
        setStructureDropTargetId((prev) => (prev === dropTargetKey ? prev : dropTargetKey));
        return;
      }
      ...
      if (draggedOuId === effectiveTargetOuId) {
        return;
      }
      const draggedRow = organizationStructureItems.find((r) => r.organUnitChildId === draggedOuId);
      const targetRow =
        dropTargetKey === ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY
          ? null
          : organizationStructureItems.find((r) => r.organUnitChildId === effectiveTargetOuId);
      if (dropTargetKey !== ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY && !targetRow) {
        return;
      }
      if (draggedRow && targetRow) {
        const dh = String(draggedRow.organUnitHierarchy ?? "");
        const th = String(targetRow.organUnitHierarchy ?? "");
        if (th.startsWith(`${dh}.`)) {
          return;
        }
      }
    ...
    [
      findOrganizationStructureEmployeeLinkByOrganId,
      organizationStructureItems,
      resolveSelectedStructureRootOrganUnitId,
  ...
  const handleOrganizationStructureParentDrop = useCallback(
    async (newParentOrganUnitId) => {
      const rootId = resolveSelectedStructureRootOrganUnitId();
      const draggedOuId = String(structureDraggedOrganUnitIdRef.current ?? "").trim();
      const draggedEmpOrgId = String(structureDraggedEmployeeOrganIdRef.current ?? "").trim();
      const parentId = String(newParentOrganUnitId ?? "").trim();
      try {
        if (draggedEmpOrgId) {
          if (!parentId || !rootId) {
            return;
          ...
          if (!link || !link.employeeId) {
            showSystemErrorToast("Не удалось найти запись сотрудника");
            return;
          }
          if (String(link.organUnitChildId) === parentId) {
            return;
          }
          const posId = String(link.employeePositionId ?? "").trim();
          const bossId = String(link.employeeParentId ?? "").trim();
          const response = await fetch(
            `${ADMIN_API_BASE_URL}/employee-position/${encodeURIComponent(draggedEmpOrgId)}`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json"
              ...
              body: JSON.stringify({
                ...
                organUnitId: parentId,
                employeePositionId: posId || null,
                bossEmployeeId: bossId || null
              })
            }
          );
          ...
          if (!response.ok) {
            showSystemErrorToast(
              String(data?.error ?? "").trim() ||
                `Не удалось сменить подразделение (HTTP ${response.status})`
            );
            return;
          }
          const updatedPosition = normalizeEmployeePositionItem(data?.item);
          if (
            updatedPosition &&
            String(link.employeeId ?? "").trim() === String(selectedEmployeeId ?? "").trim()
          ) {
            setSelectedEmployeeSnapshot((prev) => {
              ...
              const idx = positions.findIndex(
                (p) =>
                  String(p?.employeeOrganId ?? "").trim() ===
                  String(updatedPosition.employeeOrganId ?? "").trim()
              );
              if (idx >= 0) {
                positions[idx] = { ...positions[idx], ...updatedPosition };
              } else {
          ...
          setEmployeesListRefreshToken((t) => t + 1);
          await fetchOrganizationStructure(rootId);
          showSystemSuccessToast("Подразделение сотрудника обновлено");
          return;
        }

        if (!draggedOuId || !parentId || !rootId) {
          return;
        }
        if (draggedOuId === parentId) {
          return;
        }
        const draggedRow = organizationStructureItems.find((r) => r.organUnitChildId === draggedOuId);
        const targetRow =
          parentId === rootId ? null : organizationStructureItems.find((r) => r.organUnitChildId === parentId);
        if (parentId !== rootId && !targetRow) {
          showSystemErrorToast("Не удалось определить целевое подразделение");
          return;
        }
        if (draggedRow && targetRow) {
          const dh = String(draggedRow.organUnitHierarchy ?? "");
          const th = String(targetRow.organUnitHierarchy ?? "");
          if (th.startsWith(`${dh}.`)) {
            showSystemErrorToast("Нельзя подчинить подразделение своему потомку");
            return;
          }
        }
        const response = await fetch(ORGANIZATION_PARENT_PATCH_API_PATH(draggedOuId), {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json"
          },
          body: JSON.stringify({ organUnitParentId: parentId })
        });
        const data = await response.json().catch(() => ({}));
        if (!response.ok) {
          showSystemErrorToast(
            String(data?.error ?? "").trim() || `Не удалось изменить подчинение (HTTP ${response.status})`
          );
          return;
        }
        await fetchOrganizationStructure(rootId);
        showSystemSuccessToast("Подчинение обновлено");
      } catch {
        showSystemErrorToast(
          draggedEmpOrgId ? "Не удалось сменить подразделение" : "Не удалось изменить подчинение"
        );
      } finally {
        stopOrganizationStructureDragAutoScroll();
        setStructureDropTargetId("");
        if (draggedEmpOrgId) {
          structureDraggedEmployeeOrganIdRef.current = "";
        }
      }
    },
    [
      ...
      resolveSelectedStructureRootOrganUnitId,
      selectedEmployeeId,
      setEmployeesListRefreshToken,
      setSelectedEmployeeSnapshot,
      showSystemErrorToast,
  ...
  const openOrganizationStructureAddModal = useCallback(() => {
    const rootId = resolveSelectedStructureRootOrganUnitId();
    const selectedId = String(organizationStructureSelectedRowId ?? "").trim();
    let initialParentOrganUnitId = rootId;
    if (
      selectedId &&
      selectedId !== rootId &&
      organizationStructureItems.some((row) => row.organUnitChildId === selectedId)
    ) {
      initialParentOrganUnitId = selectedId;
    }
    setOrganizationStructureEditor({
      mode: "add",
      initialName: "",
      initialCode: "",
      initialKcehNumber: "",
    ...
    organizationStructureItems,
    organizationStructureSelectedRowId,
    resolveSelectedStructureRootOrganUnitId
  ]);

  const openOrganizationStructureEditModal = useCallback(
    (organUnitChildId) => {
      const id = String(organUnitChildId ?? "").trim();
      const item = organizationStructureItems.find((row) => row.organUnitChildId === id);
      const rootId = resolveSelectedStructureRootOrganUnitId();
      const initialParentOrganUnitId =
        item && rootId
          ? resolveStructureRowParentOrganUnitId(item, rootId, organizationStructureItems)
          : rootId;
      setOrganizationStructureEditor({
        mode: "edit",
        organUnitChildId: id,
        initialName: String(item?.organUnitName ?? "").trim(),
        initialCode: String(item?.organUnitCode ?? "").trim(),
        initialKcehNumber: String(item?.kcehNumber ?? "").trim(),
        initialParentOrganUnitId: initialParentOrganUnitId || rootId
      });
    },
    [organizationStructureItems, resolveSelectedStructureRootOrganUnitId]
  );

  const closeOrganizationStructureEditorModal = useCallback(() => {
    if (organizationStructureEditorSaving) {
      return;
    }
    setOrganizationStructureEditor(null);
  }, [organizationStructureEditorSaving]);

  const submitOrganizationStructureEditorModal = useCallback(async (form) => {
    if (!organizationStructureEditor) {
      return;
    }
    const rootId = resolveSelectedStructureRootOrganUnitId();
    if (!rootId) {
      showSystemErrorToast("Не удалось определить организацию");
      return;
    }
    const name = String(form?.name ?? "").trim();
    if (!name) {
      showSystemErrorToast("Укажите наименование подразделения");
      return;
    }
    const code = String(form?.code ?? "").trim();
    const kcehRaw = String(form?.kcehNumber ?? "").trim();

    const excludeId =
      organizationStructureEditor?.mode === "edit"
        ? String(organizationStructureEditor.organUnitChildId ?? "").trim()
        : "";
    if (kcehRaw) {
      const conflict = organizationStructureItems.find((row) => {
        if (excludeId && row.organUnitChildId === excludeId) {
          return false;
        }
        return String(row.kcehNumber ?? "").trim() === kcehRaw;
      });
      if (conflict) {
        showSystemErrorToast("Подразделение с таким номером цеха уже есть в организации");
        return;
      }
    }

    const resolveValidParentOrganUnitId = (rawParent) => {
      const parentOrganUnitId = String(rawParent ?? "").trim();
      if (!parentOrganUnitId) {
        return "";
      }
      const valid =
        parentOrganUnitId === rootId ||
        organizationStructureItems.some((row) => row.organUnitChildId === parentOrganUnitId);
      return valid ? parentOrganUnitId : "";
    };

    setOrganizationStructureEditorSaving(true);
    try {
      if (organizationStructureEditor?.mode === "add") {
        const parentOrganUnitId = resolveValidParentOrganUnitId(form?.parentOrganUnitId);
        if (!parentOrganUnitId) {
          showSystemErrorToast("Выберите допустимую родительскую структуру");
          return;
        }
        const body = {
          name,
          structureRootOrganUnitId: rootId
        };
        if (code) {
          body.code = code;
        }
        if (kcehRaw) {
          body.kcehNumber = kcehRaw;
        }
        const response = await fetch(ORGANIZATION_SUBDIVISIONS_CREATE_API_PATH(parentOrganUnitId), {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
        ...
        if (!response.ok) {
          showSystemErrorToast(
            String(data?.error ?? "").trim() || `Не удалось создать подразделение (HTTP ${response.status})`
          );
          return;
        }
        setOrganizationStructureEditor(null);
        await fetchOrganizationStructure(rootId);
        showSystemSuccessToast("Подразделение добавлено");
        return;
      }

      if (organizationStructureEditor?.mode === "edit") {
        const childId = String(organizationStructureEditor.organUnitChildId ?? "").trim();
        if (!childId) {
          showSystemErrorToast("Не удалось определить подразделение");
          return;
        }
        const newParentOrganUnitId = resolveValidParentOrganUnitId(form?.parentOrganUnitId);
        if (!newParentOrganUnitId) {
          showSystemErrorToast("Выберите допустимую родительскую структуру");
          return;
        }
        const editedRow = organizationStructureItems.find((row) => row.organUnitChildId === childId);
        const currentParentOrganUnitId = editedRow
          ? resolveStructureRowParentOrganUnitId(editedRow, rootId, organizationStructureItems)
          : rootId;
        const payload = {
          name,
          shName: name,
          code: code || null,
          kcehNumber: kcehRaw || null,
          structureRootOrganUnitId: rootId
        };
        const response = await fetch(ORGANIZATION_UPDATE_API_PATH(encodeURIComponent(childId)), {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json"
          },
        ...
        if (!response.ok) {
          showSystemErrorToast(
            String(data?.error ?? "").trim() || `Не удалось сохранить подразделение (HTTP ${response.status})`
          );
          return;
        }
        if (newParentOrganUnitId !== currentParentOrganUnitId) {
          const parentResponse = await fetch(
            ORGANIZATION_PARENT_PATCH_API_PATH(encodeURIComponent(childId)),
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json"
              },
              body: JSON.stringify({ organUnitParentId: newParentOrganUnitId })
            }
          );
          const parentData = await parentResponse.json().catch(() => ({}));
          if (!parentResponse.ok) {
            showSystemErrorToast(
              String(parentData?.error ?? "").trim() ||
                `Не удалось изменить родителя (HTTP ${parentResponse.status})`
            );
            await fetchOrganizationStructure(rootId);
            return;
          }
        ...
        setOrganizationStructureEditor(null);
        await fetchOrganizationStructure(rootId);
        showSystemSuccessToast("Подразделение сохранено");
      }
    } catch {
      showSystemErrorToast("Не удалось сохранить подразделение");
    } finally {
      setOrganizationStructureEditorSaving(false);
    }
  }, [
    fetchOrganizationStructure,
    organizationStructureEditor,
  ...
  const requestDeleteOrganizationSubdivision = useCallback(
    (organUnitChildId) => {
      const id = String(organUnitChildId ?? "").trim();
      if (!id) {
        return;
      ...
      if (employees.length > 0) {
        showSystemErrorToast("Невозможно удалить подразделение: в нём закреплены сотрудники");
        return;
      }
      setPendingOrganizationSubdivisionDelete({
        organUnitChildId: id,
        name: String(item?.organUnitName ?? "").trim()
      });
    },
    [organizationStructureItems, showSystemErrorToast]
  );

  const closeDeleteOrganizationSubdivisionModal = useCallback(() => {
    setPendingOrganizationSubdivisionDelete(null);
  }, []);

  const confirmDeleteOrganizationSubdivision = useCallback(async () => {
    if (!pendingOrganizationSubdivisionDelete?.organUnitChildId) {
      return;
    }
    const rootId = resolveSelectedStructureRootOrganUnitId();
    const target = pendingOrganizationSubdivisionDelete;
    setPendingOrganizationSubdivisionDelete(null);
    try {
      const response = await fetch(
        `${ADMIN_API_BASE_URL}/organization/${encodeURIComponent(target.organUnitChildId)}`,
        { method: "DELETE" }
      );
      ...
      if (!response.ok) {
        const errorMessage =
          ...
          `Не удалось удалить подразделение (HTTP ${response.status})`;
        showSystemErrorToast(errorMessage);
        return;
      }
      if (rootId) {
      ...
      showSystemSuccessToast("Подразделение удалено");
    } catch {
      showSystemErrorToast("Не удалось удалить подразделение");
    }
  }, [
    fetchOrganizationStructure,
    pendingOrganizationSubdivisionDelete,
    resolveSelectedStructureRootOrganUnitId,
    showSystemErrorToast,
    showSystemSuccessToast
  ]);

  const handleOrganizationStructureResizeStart = useCallback(
    (boundary, event) => {
      event.preventDefault();
      event.stopPropagation();
      const wrap = organizationStructureTableWrapperRef.current;
      if (!wrap) {
        return;
      }
      const fw = Math.max(
        ORGANIZATION_STRUCTURE_MIN_DATA_COL_PX * 3,
        wrap.clientWidth -
          ORGANIZATION_STRUCTURE_ACTIONS_COL_PX -
          ORGANIZATION_STRUCTURE_WRAPPER_GUTTER_PX
      );
      organizationStructureResizeRef.current = {
        boundary,
        startX: event.clientX,
        startFracs: { ...organizationStructureColFracs },
        fw
      };
      document.body.style.cursor = "col-resize";
      document.body.style.userSelect = "none";
      const handleMouseMove = (moveEvent) => {
        const ctx = organizationStructureResizeRef.current;
        if (!ctx) {
          return;
        }
        const delta = moveEvent.clientX - ctx.startX;
        const deltaFrac = delta / ctx.fw;
        const minF = ORGANIZATION_STRUCTURE_MIN_DATA_COL_PX / ctx.fw;
        const s = ctx.startFracs;
        setOrganizationStructureColFracs((prev) => {
          if (ctx.boundary === "name-code") {
            let name = s.name + deltaFrac;
            let code = s.code - deltaFrac;
            name = Math.max(minF, Math.min(name, s.name + s.code - minF));
            code = s.name + s.code - name;
            return { name, code, kceh: s.kceh };
          }
          if (ctx.boundary === "code-kceh") {
            let code = s.code + deltaFrac;
            let kceh = s.kceh - deltaFrac;
            code = Math.max(minF, Math.min(code, s.code + s.kceh - minF));
            kceh = s.code + s.kceh - code;
            return { name: s.name, code, kceh };
          }
          if (ctx.boundary === "kceh-name") {
            let kceh = s.kceh + deltaFrac;
            let name = s.name - deltaFrac;
            kceh = Math.max(minF, Math.min(kceh, s.kceh + s.name - minF));
            name = s.kceh + s.name - kceh;
            return { name, code: s.code, kceh };
          }
          return prev;
        });
      };
      const handleMouseUp = () => {
        organizationStructureResizeRef.current = null;
        document.body.style.cursor = "";
        document.body.style.userSelect = "";
        window.removeEventListener("mousemove", handleMouseMove);
        window.removeEventListener("mouseup", handleMouseUp);
  ...
  const handleOrganizationStructurePanesResizeStart = useCallback(
    (event) => {
      event.preventDefault();
      event.stopPropagation();
      const layout = organizationStructureLayoutRef.current;
      if (!(layout instanceof HTMLElement)) {
        return;
      }
      const rect = layout.getBoundingClientRect();
      const innerW = rect.width - ORGANIZATION_STRUCTURE_PANES_HANDLE_PX;
      const minTotal = ORGANIZATION_STRUCTURE_PANES_MIN_LEFT_PX + ORGANIZATION_STRUCTURE_PANES_MIN_RIGHT_PX;
      if (innerW < minTotal) {
        return;
      }
      organizationStructurePanesResizeRef.current = {
        startX: event.clientX,
        startLeftPx: innerW * organizationStructurePanesSplitFrac,
        innerWAtStart: innerW
      };
      document.body.style.cursor = "col-resize";
      document.body.style.userSelect = "none";
      const handleMouseMove = (moveEvent) => {
        const ctx = organizationStructurePanesResizeRef.current;
        const layoutEl = organizationStructureLayoutRef.current;
        if (!ctx || !(layoutEl instanceof HTMLElement)) {
          return;
        }
        const r = layoutEl.getBoundingClientRect();
        const innerNow = Math.max(minTotal, r.width - ORGANIZATION_STRUCTURE_PANES_HANDLE_PX);
        const dx = moveEvent.clientX - ctx.startX;
        let leftPx = ctx.startLeftPx + dx;
        const maxLeft = innerNow - ORGANIZATION_STRUCTURE_PANES_MIN_RIGHT_PX;
        const minLeft = ORGANIZATION_STRUCTURE_PANES_MIN_LEFT_PX;
        leftPx = Math.max(minLeft, Math.min(maxLeft, leftPx));
        const frac = sanitizeOrganizationStructurePanesSplit(innerNow > 0 ? leftPx / innerNow : 0.5);
        setOrganizationStructurePanesSplitFrac(frac);
      };
      const handleMouseUp = () => {
        organizationStructurePanesResizeRef.current = null;
        document.body.style.cursor = "";
        document.body.style.userSelect = "";
        window.removeEventListener("mousemove", handleMouseMove);
        window.removeEventListener("mouseup", handleMouseUp);
  ...
  const getDragClientX = useCallback((event) => {
    const directClientX = Number(event?.clientX);
    if (Number.isFinite(directClientX)) {
      return directClientX;
    }
    const nativeClientX = Number(event?.nativeEvent?.clientX);
    if (Number.isFinite(nativeClientX)) {
      return nativeClientX;
    }
    const pageX = Number(event?.pageX ?? event?.nativeEvent?.pageX);
  ...
  useEffect(() => {
    const handleWindowDragOver = (event) => {
      if (!structureDraggedOrganUnitIdRef.current && !structureDraggedEmployeeOrganIdRef.current) {
        return;
      }
      ...
      updateOrganizationStructureDragAutoScroll(clientX, clientY);
    };
    const handleWindowDrag = (event) => {
      if (!structureDraggedOrganUnitIdRef.current && !structureDraggedEmployeeOrganIdRef.current) {
        return;
  ...
  }, [getDragClientX, getDragClientY, updateOrganizationStructureDragAutoScroll]);
  useEffect(() => () => stopOrganizationStructureDragAutoScroll(), [stopOrganizationStructureDragAutoScroll]);
  useEffect(() => {
    if (!isOrganizationsPage || !isOrganizationCardPanelOpen) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (activeOrganizationCardTab !== ORGANIZATION_CARD_TABS.STRUCTURE) {
      return;
    }
    const organizationId = String(
      selectedOrganization?.organUnitId ??
    ...
    void fetchOrganizationStructure(organizationId);
  }, [
    activeOrganizationCardTab,
    fetchOrganizationStructure,
    isOrganizationCardPanelOpen,
    isOrganizationsPage,
    ORGANIZATION_CARD_TABS.STRUCTURE,
    selectedOrganization?.id,
    selectedOrganization?.organUnitId,
    selectedOrganization?.organ_unit_id,
    selectedOrganizationId
  ...
  useEffect(() => {
    setOrganizationStructureCollapsedIds(new Set());
    setOrganizationStructureSelectedRowId("");
    setStructureEmployeesSubdivisionFilterSync((prev) => ({
      tick: prev.tick + 1,
      value: ""
    }));
  ...
  useLayoutEffect(() => {
    if (!isOrganizationCardPanelOpen || activeOrganizationCardTab !== ORGANIZATION_CARD_TABS.STRUCTURE) {
      return;
    }
    const element = organizationStructureTableWrapperRef.current;
    if (!element) {
      return;
    }
    const measure = () => {
      ...
      setOrganizationStructureFluidWidth((prev) => (prev === next ? prev : next));
    };
    measure();
  ...
  const handleCellMouseEnter = useCallback(
    (event, value, tooltipOverride = null, alwaysShowTooltip = false, measurementRoot = null) => {
    const valueText = String(value ?? "").trim();
    const overrideTrimmed = tooltipOverride != null ? String(tooltipOverride).trim() : "";
    const tooltipText = overrideTrimmed || valueText;
    if (!tooltipText || tooltipText === "-") {
      setCellTooltip((prev) => ({ ...prev, visible: false, text: "" }));
      return;
    }

    const root = measurementRoot ?? event.currentTarget;
    const link = root instanceof HTMLElement ? root.querySelector("a.employee-name-link") : null;
    /* Ячейка + ссылка: ellipsis может быть на td или на inline-block a; оба варианта проверяем.
       Раньше мерили только <a> — у части браузеров scrollWidth === clientWidth при обрезке. */
    const elOverflow = (el) =>
      el instanceof HTMLElement && Math.ceil(el.scrollWidth) > Math.floor(el.clientWidth);
    const overflow =
      elOverflow(root) || (link instanceof HTMLElement && elOverflow(link));

    const shouldShow = alwaysShowTooltip || overflow;
    if (!shouldShow) {
      setCellTooltip((prev) => ({ ...prev, visible: false, text: "" }));
      return;
    }
  ...
  const getOrganizationCardRelationRowSelectionKey = (row) => {
    const rid = String(row?.relationId ?? "").trim();
    if (rid) {
      return rid;
    }
    return [
      String(row?.employeeId ?? "").trim(),
      String(row?.relationTypeId ?? "").trim(),
      String(row?.salesOrganizationId ?? "").trim(),
      String(row?.productGroupsId ?? "").trim()
    ].join("::");
  };

  const getEmployeeCardRelationRowSelectionKey = (row) => {
    const rid = String(row?.relationId ?? "").trim();
    if (rid) {
      return rid;
    }
    return [
      String(row?.organUnitId ?? "").trim(),
      String(row?.relationTypeId ?? "").trim(),
      String(row?.salesOrganizationId ?? "").trim(),
      String(row?.productGroupsId ?? "").trim()
    ].join("::");
  ...
  const clearOrganizationCardRelationFilters = () => {
    setOrganizationCardRelationsFilters(INITIAL_ORGANIZATION_RELATION_FILTERS);
  };

  const getOrganizationCardRelationSortDirectionForField = (field) => {
    const match = organizationCardRelationsSortRules.find((rule) => rule.field === field);
    return match ? match.direction : null;
  };

  const getOrganizationCardRelationSortOrderForField = (field) => {
    const sortIndex = organizationCardRelationsSortRules.findIndex((rule) => rule.field === field);
    return sortIndex >= 0 ? sortIndex + 1 : null;
  };

  const handleOrganizationCardRelationSortClick = (field) => {
    setOrganizationCardRelationsSortRules((prev) => {
      const index = prev.findIndex((rule) => rule.field === field);
      if (index === -1) {
        return [...prev, { field, direction: "ASC" }];
      }
  ...
  const handleOrganizationCardRelationFilterChange = (field, value) => {
    setOrganizationCardRelationsFilters((prev) => ({
      ...prev,
      [field]: value
    }));
  };
  ...
  const fetchOrganizationOptions = async (filterValue, showShortCode) => {
    ...
    if (!showShortCode) {
      params.set("relationOrganLookup", "true");
    }
    if (effectiveFilter) {
  ...
  const fetchPositionEmployeeOptions = async (filterValue, departUnitId) => {
    ...
    try {
      ...
      const data = await response.json();
      if (String(latestEmployeePositionFormRef.current?.departUnitId ?? "").trim() !== normalizedDepartUnitId) {
        return;
      }
      if (!response.ok) {
        setEmployeeRelationsError(data.error ?? "Не удалось получить список сотрудников");
  ...
  const openAddEmployeeRelationRow = async () => {
    const relationTargetRef = isEmployeeRelationsPage
      ? tableWrapperRef
      : isOrganizationCardRelationsTab
        ? organizationCardRelationsTableWrapperRef
        : relationTableWrapperRef;
    ...
    if (isOrganizationCardRelationsTab) {
      ...
      nextForm.organUnitId = orgId;
      nextForm.organName = orgName;
      nextForm.organNameFilter = orgName;
      nextForm.organSapId = String(selectedOrganization?.sapId ?? selectedOrganization?.sap_id ?? "").trim();
      nextForm.organInn = String(selectedOrganization?.inn ?? "").trim();
      nextForm.organKpp = String(selectedOrganization?.kpp ?? "").trim();
      nextForm.organOgrn = String(selectedOrganization?.ogrn ?? "").trim();
      nextForm.organFullAddress = String(selectedOrganization?.address ?? "").trim();
      lastConfirmedOrganRef.current = {
        id: orgId,
        name: orgName,
        sapId: nextForm.organSapId,
        inn: nextForm.organInn,
        kpp: nextForm.organKpp,
        ogrn: nextForm.organOgrn,
        fullAddress: nextForm.organFullAddress
      };
    } else {
      lastConfirmedOrganRef.current = {
        id: "",
    ...
    setNewEmployeeRelationForm(nextForm);
    ...
    await Promise.all(
      [
        isEmployeeRelationsPage || isOrganizationCardRelationsTab ? fetchRelationEmployeeOptions("") : null,
        fetchRelationTypeOptions(""),
        isOrganizationCardRelationsTab ? null : fetchOrganizationOptions("", false),
  ...
  const openEditEmployeePositionRow = async (positionRow) => {
    ...
    const formState = {
      ...
      editInitialDepartUnitId: String(positionRow?.departUnitId ?? "").trim(),
  ...
  const saveEmployeePosition = async () => {
    ...
    const bossIdForSave = String(newEmployeePositionForm.bossEmployeeId ?? "").trim();
    if (bossIdForSave) {
      const bossInList = positionEmployeeOptions.some((b) => String(b.id) === bossIdForSave);
      if (positionEmployeeOptions.length > 0 && !bossInList) {
        const bossMsg = "В новом подразделении выберите руководителя заново";
        showEmployeeRelationsError(bossMsg);
        showSystemErrorToast(bossMsg);
        return;
      }
    }
  ...
  const saveNewEmployeeRelation = async () => {
    ...
    const organUnitIdForSave = isOrganizationCardRelationsTab
      ? String(selectedOrganizationId ?? "").trim()
      : String(newEmployeeRelationForm.organUnitId ?? "").trim();
    if (isOrganizationCardRelationsTab && !organUnitIdForSave) {
      setEmployeeRelationsError("Не удалось определить организацию");
      return;
    }
    const requiredMappings = [
      ...(isEmployeeRelationsPage || isOrganizationCardRelationsTab ? [["employeeId", "ФИО сотрудника"]] : []),
      ...(!isOrganizationCardRelationsTab ? [["organUnitId", "Организация"]] : []),
    ...
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify(
          toCamelApiPayload({
            ...
            organUnitId: organUnitIdForSave,
      ...
      if (data?.item && typeof data.item === "object") {
        ...
        if (isEditMode) {
          ...
          } else if (isOrganizationCardRelationsTab) {
            setOrganizationCardRelations((prev) =>
              prev.map((row) => (getRelationId(row) === editingEmployeeRelationId ? normalizedItem : row))
            );
          } else {
          ...
          if (isEmployeeRelationsPage) {
            ...
            setOrganizationCardRelations((prev) => [normalizedItem, ...prev]);
          } else {
  ...
  const updateRelationComboMenuLayout = (comboKey) => {
    ...
    const tabContentElement =
      (isOrganizationCardRelationsTab && organizationCardRelationsTabContentRef.current) ||
      employeeRelationsTabContentRef.current ||
    ...
    setRelationComboMenuLayouts((prev) => ({
      ...
      [comboKey]: {
        ...
        themeVars: {
          ...
          "--muted-text": inputComputedStyle.getPropertyValue("--muted-text").trim(),
  ...
  useEffect(() => {
    if (!organizationCardRelationsError) {
      return;
    }
    showSystemErrorToast(organizationCardRelationsError);
  }, [organizationCardRelationsError, showSystemErrorToast]);
  ...
  const renderEmployeeRelationFormRow = (cancelTitle, rowKey) => (
    <tr className="employee-card-relations-edit-row" key={rowKey}>
      {(isEmployeeRelationsPage || isOrganizationCardRelationsTab) && (
        <td>
      ...
      <td
        className={`employee-card-relations-actions-cell${
          isEmployeeRelationsPage || isOrganizationCardRelationsTab ? " relations-list-actions-cell" : ""
        }`}
      >
        <button
          type="button"
  ...
  const openEditEmployeeRelationRowRef = useRef(openEditEmployeeRelationRow);
  openEditEmployeeRelationRowRef.current = openEditEmployeeRelationRow;
  const stableOpenEditEmployeeRelationRow = useCallback((relationRow) => {
    void openEditEmployeeRelationRowRef.current(relationRow);
  }, []);

  const openDeleteRelationModalRef = useRef(openDeleteRelationModal);
  openDeleteRelationModalRef.current = openDeleteRelationModal;
  const stableOpenDeleteRelationModal = useCallback((relationRow) => {
    openDeleteRelationModalRef.current(relationRow);
  }, []);

  const setSelectedRowIndexRef = useRef(setSelectedRowIndex);
  setSelectedRowIndexRef.current = setSelectedRowIndex;
  const stableHandleRelationsListRowClick = useCallback((rowIndex) => {
    setSelectedRowIndexRef.current(rowIndex);
  }, []);
  ...
  const getOrganizationCardRelationColumnWidthPx = (columnKey) => {
    const rawWidth = Number(
      organizationCardRelationColumnWidths[columnKey] ??
        DEFAULT_ORGANIZATION_RELATION_COLUMN_WIDTHS[columnKey] ??
        MIN_COLUMN_WIDTH
    );
    if (!Number.isFinite(rawWidth)) {
      return MIN_COLUMN_WIDTH;
  ...
  const organizationCardRelationTableWidthPx =
    ORGANIZATION_RELATION_COLUMNS.reduce(
      (sum, column) => sum + getOrganizationCardRelationColumnWidthPx(column.key),
      0
    ) + 96;

  const handleOrganizationCardRelationResizeStart = (field, event) => {
    event.preventDefault();
    event.stopPropagation();

    const startWidth = getOrganizationCardRelationColumnWidthPx(field);
    relationResizeStateRef.current = {
      field,
      startX: event.clientX,
      startWidth
    ...
    const handleMouseMove = (moveEvent) => {
      ...
      const delta = moveEvent.clientX - relationResizeStateRef.current.startX;
      const nextWidth = Math.max(MIN_COLUMN_WIDTH, relationResizeStateRef.current.startWidth + delta);
      setOrganizationCardRelationColumnWidths((prev) => ({
        ...prev,
        [field]: nextWidth
      }));
    };
  ...
  const handleAlignOrganizationCardRelationColumns = () => {
    const wrapper = organizationCardRelationsTableWrapperRef.current;
    if (!wrapper) {
      return;
    }
    ...
    const renderedColumns = ORGANIZATION_RELATION_COLUMNS.map((column) => ({
      key: column.key,
      width: getOrganizationCardRelationColumnWidthPx(column.key)
    }));
    const currentTotalWidth = renderedColumns.reduce((sum, column) => sum + column.width, 0);
    if (currentTotalWidth <= 0) {
      return;
  ...
  return (
    <main className="app-layout">
      <aside className={`sidebar${isSidebarCollapsed ? " collapsed" : ""}`}>
        <div className="sidebar-top">
          <button
            ...
            data-tooltip="Администрирование"
          >
          ...
          <button
            ...
            data-tooltip="Шаблоны печатных форм"
          >
            <span className="sidebar-action-icon" aria-hidden="true">
              <svg viewBox="0 0 24 24" focusable="false">
        ...
        <div className="sidebar-bottom">
          <button
            ...
            data-tooltip={isDarkThemeEnabled ? "Выключить темную тему" : "Включить темную тему"}
          >
            <span className="sidebar-action-icon" aria-hidden="true">
              <svg viewBox="0 0 24 24" focusable="false">
          ...
          <button
            ...
            data-tooltip={isSidebarCollapsed ? "Развернуть" : "Свернуть"}
          >
            <span className="sidebar-action-icon" aria-hidden="true">
              <svg viewBox="0 0 24 24" focusable="false">
                {isSidebarCollapsed ? (
      ...
      {(isAdministrationOpen || isAdministrationClosing) &&
        createPortal(
          <div
            className="administration-modal-overlay"
            role="presentation"
            onClick={closeAdministrationPanel}
          >
            <aside
              className={`administration-panel${isDarkThemeEnabled ? " dark" : ""}${
                isAdministrationClosing ? " closing" : ""
              }`}
              role="dialog"
              aria-modal="true"
              aria-label="Администрирование"
            ...
            >
              <header className="administration-panel-title administration-panel-title-with-actions">
                <span className="administration-panel-title-label">Администрирование</span>
                <div className="administration-panel-title-actions">
                  <button
                    type="button"
                    className="administration-panel-title-icon-btn"
                    aria-label="Развернуть все"
                    data-tooltip="Развернуть все"
                    onClick={() => {
                      setIsReferenceBooksSectionExpanded(true);
                      setIsParticipantsExpanded(true);
                      setIsReportBuilderExpanded(true);
                    }}
                  >
                    <IconChevronsDown aria-hidden />
                  </button>
                  <button
                    type="button"
                    className="administration-panel-title-icon-btn"
                    ...
                    onClick={() => {
                      setIsReferenceBooksSectionExpanded(false);
                      setIsParticipantsExpanded(false);
                      setIsReportBuilderExpanded(false);
                    }}
                  >
              ...
              <button
                ...
                onClick={() => setIsReferenceBooksSectionExpanded((prev) => !prev)}
                aria-expanded={isReferenceBooksSectionExpanded}
              >
                <span>Справочники/Классификаторы</span>
                <span className="administration-section-chevron" aria-hidden="true">
                  {isReferenceBooksSectionExpanded ? "▾" : "▸"}
                </span>
              </button>
              {isReferenceBooksSectionExpanded && (
                <div className="administration-section-items">
                  <button
                    ...
                    className={`administration-section-item${
                      isReferenceBooksPage || isReferenceBookDataPage ? " active" : ""
                    }`}
                    onClick={() => handlePageSelect(PAGE_IDS.REFERENCE_BOOKS)}
                  >
                    <IconListBulleted aria-hidden />
                    <span>Список справочников</span>
                  </button>
                </div>
              )}
              <button
              ...
              {isParticipantsExpanded && (
                <div className="administration-section-items">
                  ...
                  >
                    ...
                    <span>Список связей сотрудников</span>
                  </button>
                </div>
              )}
              <button
              ...
              {isReportBuilderExpanded && (
                <div className="administration-section-items">
                  <button
                    ...
                    onMouseEnter={prefetchReportSettingsPage}
                  >
                    <IconSliders aria-hidden />
                    <span>Настройка отчетов</span>
                  </button>
                </div>
              )}
      ...
      <section className={`content-area${isDarkThemeEnabled ? " dark" : ""}`}>
        <header
          className={`page-header${
            isReportHeaderContext ||
            isEmployeeRelatedHeaderContext ||
            isReferenceBooksPage ||
            isReferenceBookDataPage
              ? " report-settings-page-header"
              : ""
          }`}
        >
        ...
        {shouldRenderHeaderActions && (
          <div className={`page-header-actions-row report-list-header-actions mode-${headerActionsMode}`} key={headerActionsMode}>
            {isReportHeaderContext ? (
              <>
              ...
              <>
                ...
                <ExportToExcelButton
                  ...
                  onError={(error) =>
                    setEmployeesError(
                      error instanceof Error ? error.message : "Ошибка выгрузки связей сотрудников"
                    )
                  }
                />
                <button
              ...
              <>
                ...
                <button
                  type="button"
                  className="panel-action-button"
                  aria-label="Очистить фильтры таблицы организаций"
              ...
              <>
                <button type="button" className="panel-action-button" onClick={handleBackFromReferenceBookData}>
                  <IconChevronLeft aria-hidden />
                  <span>К списку справочников</span>
                </button>
                {referenceBookDataToolbar.addRecords ? (
                  <button type="button" className="panel-action-button" onClick={invokeReferenceBookDataOpenAdd}>
                    <IconPlus aria-hidden />
                    <span>Добавить</span>
                  </button>
                ) : null}
                <ExportToExcelButton
                  exportFile={invokeReferenceBookDataExportFile}
                  disabled={false}
                  className="panel-action-button"
                  onError={(error) =>
                    showSystemErrorToast(error instanceof Error ? error.message : "Ошибка выгрузки")
                  }
                />
                <button
                  type="button"
                  className="panel-action-button"
                  onClick={invokeReferenceBookDataClearFilters}
                >
                  <IconClose aria-hidden />
                  <span>Удалить фильтр</span>
                </button>
                <button type="button" className="panel-action-button" onClick={invokeReferenceBookDataAlignColumns}>
                  <IconAlignColumns aria-hidden />
                  <span>Выровнять</span>
                </button>
                <button
                  type="button"
                  className="panel-action-button"
                  onClick={invokeReferenceBookDataOpenColumnSettings}
                >
                  <IconSettings aria-hidden />
                  <span>Настройка</span>
                </button>
              ...
              <>
                <button
                  type="button"
                  className="panel-action-button"
                  onClick={invokeReferenceBooksOpenAdd}
                >
                  <IconPlus aria-hidden />
                  <span>Добавить</span>
                </button>
                <button
                  type="button"
                  className="panel-action-button"
                  onClick={invokeReferenceBooksClearFilters}
                >
                  <IconClose aria-hidden />
                  <span>Удалить фильтр</span>
                </button>
                <button
                  type="button"
                  className="panel-action-button"
                  onClick={invokeReferenceBooksAlignColumns}
                >
                  <IconAlignColumns aria-hidden />
                  <span>Выровнять</span>
                </button>
                <button
                  type="button"
                  className="panel-action-button"
                  onClick={invokeReferenceBooksOpenColumnSettings}
                >
                  <IconSettings aria-hidden />
                  <span>Настройка</span>
                </button>
        ...
        <section
          className={`main-panel${
            isReportHeaderContext || isEmployeeRelatedHeaderContext || isReferenceBooksPage || isReferenceBookDataPage
              ? " report-settings-main-panel"
              : ""
          }${isReferenceBooksPage || isReferenceBookDataPage ? " reference-books-main-panel" : ""}`}
          ref={mainPanelRef}
        >
          {isReferenceBookDataPage ? (
            <ReferenceBookDataPage
              referenceBookUrlSuffix={referenceBookDataUrlSuffix}
              listApiUrl={REFERENCE_BOOKS_LIST_API_URL}
              isDarkThemeEnabled={isDarkThemeEnabled}
              showSystemErrorToast={showSystemErrorToast}
              showSystemSuccessToast={showSystemSuccessToast}
              onToolbarMeta={setReferenceBookDataToolbar}
              onPageTitleChange={setReferenceBookDataHeaderTitle}
            ...
            <ReferenceBooksListPage
              listApiUrl={REFERENCE_BOOKS_LIST_API_URL}
              isDarkThemeEnabled={isDarkThemeEnabled}
              showSystemErrorToast={showSystemErrorToast}
              showSystemSuccessToast={showSystemSuccessToast}
              onSelectReferenceBookData={handleOpenReferenceBookData}
              settingsUrlSuffix={referenceBookSettingsUrlSuffix}
              onReferenceBookSettingsNavigate={handleReferenceBookSettingsNavigate}
            />
          ) : isListPage ? (
            isReportSettingsPage ? (
              <Suspense
                fallback={
                  <div className="list-content-layout report-settings-layout">
                    <div className="list-content-main">Загрузка экрана отчётов…</div>
                  </div>
                }
              >
                <ReportSettingsPage
                  ...
                  handleCellMouseLeave,
                  selectedRowIndex,
                  onReportRowClick: stableHandleReportRowClick
                }}
                sideCardGroups={{
                  shell: {
                    isReportCardVisible,
                  ...
                  previewTab: {
                    ...
                    isReportPreviewTabAvailable,
                    setReportPreviewError,
                    onRefreshReportPreview: handleRefreshReportPreview,
                    reportPreviewStats,
            ...
            <>
              <div
                className={`list-content-layout${isSideCardVisible ? " split-view" : ""}${
                  isEmployeeRelatedHeaderContext ? " report-settings-layout" : ""
                }`}
              >
              ...
              <tbody>
                      ...
                      renderRow={(virtualRow) => {
                        ...
                        return (
                          <OrganizationListTableRow
                            ...
                            updateCellTooltipPosition={updateCellTooltipPosition}
                            handleCellMouseLeave={handleCellMouseLeave}
                            onRowMouseDown={handleOrganizationRowMouseDown}
                            onRowClick={stableHandleOrganizationRowClick}
                          />
                        );
                      }}
                    />
                ...
                {!employeesError &&
                  ...
                  (listVirtualizationActive ? (
                      ...
                      renderRow={(virtualRow) => {
                        ...
                        return (
                          <EmployeeRelationsListTableRow
                            ...
                            onRowClick={stableHandleRelationsListRowClick}
                            onEditRelation={stableOpenEditEmployeeRelationRow}
                            onDeleteRelation={stableOpenDeleteRelationModal}
                            normalizeEmployeeId={normalizeEmployeeId}
                            normalizeOrganizationId={normalizeOrganizationId}
                            buildEmployeeCardUrl={buildEmployeeCardUrl}
                            buildOrganizationCardUrl={buildOrganizationCardUrl}
                          />
                        );
                      }}
                    />
                    ...
                    displayedEmployees.map((row, rowIndex) => {
                      if (getRelationId(row) === editingEmployeeRelationId) {
                      ...
                      renderRow={(virtualRow) => {
                        ...
                        return (
                          <EmployeesListTableRow
                            ...
                            updateCellTooltipPosition={updateCellTooltipPosition}
                            handleCellMouseLeave={handleCellMouseLeave}
                            getEmployeeDerivedPositionData={getEmployeeDerivedPositionData}
                            normalizeOrganizationId={normalizeOrganizationId}
                            buildOrganizationCardUrl={buildOrganizationCardUrl}
                            isSelected={rowEmployeeId !== "" && rowEmployeeId === selectedEmployeeId}
                            onRowClick={stableHandleEmployeeRowClick}
                          />
                        );
                      }}
                    />
                                      ...
                                      >
                                        <td
                                          onMouseEnter={(event) => {
                                            ...
                                            handleCellMouseEnter(event, positionRow?.organName ?? "-", tooltipText, true);
                                          }}
                                          onMouseMove={updateCellTooltipPosition}
                                          onMouseLeave={handleCellMouseLeave}
                                        >
                                        ...
                                        <td
                                          onMouseEnter={(event) =>
                                            handleCellMouseEnter(
                                              event,
                                              normalizeUiText(positionRow?.departName) || "-"
                                            )
                                          }
                                          onMouseMove={updateCellTooltipPosition}
                                          onMouseLeave={handleCellMouseLeave}
                                        ...
                                        <td
                                          ...
                                          onMouseLeave={handleCellMouseLeave}
                                        >
                                          {normalizeEmployeeId(positionRow?.bossId) ? (
                                            <a
                                              className="entity-card-hover-link employee-name-link"
                                              href={buildEmployeeCardUrl(positionRow?.bossId)}
                                              target="_blank"
                                              rel="noopener noreferrer"
                                              onClick={(event) => {
                                                event.stopPropagation();
                              ...
                              >
                                ...
                                <tbody>
                                  ...
                                  {employeeRelations.map((relationRow, index) => {
                                      if (getRelationId(relationRow) === editingEmployeeRelationId) {
                                        return renderEmployeeRelationFormRow(
                                          "Отменить редактирование связи",
                                          `relation-edit-${editingEmployeeRelationId || index}`
                                        );
                                      }
                                      const empRelRowKey = getEmployeeCardRelationRowSelectionKey(relationRow);
                                      const isEmpRelRowSelected = selectedEmployeeCardRelationKey === empRelRowKey;
                                      return (
                                      <tr
                                        ...
                                        className={
                                          isEmpRelRowSelected ? "employee-card-relations-row-selected" : undefined
                                        }
                                        aria-selected={isEmpRelRowSelected}
                                        onClick={(event) => {
                                          ...
                                          setSelectedEmployeeCardRelationKey(empRelRowKey);
                                        }}
                                        onMouseDown={() => {
                                          if (isRelationFormActive) {
                                            cancelAddEmployeeRelationRow();
                                      ...
                                      >
                                        <td
                                          onMouseEnter={(event) => {
                                            ...
                                            handleCellMouseEnter(event, relationRow?.organName ?? "-", tooltipText, true);
                                          }}
                                          onMouseMove={updateCellTooltipPosition}
                                          onMouseLeave={handleCellMouseLeave}
                                        >
                  ...
                  <aside className={`employee-card-panel report-card-panel${isOrganizationCardVisible ? " open" : ""}`}>
                    ...
                    <div className="employee-card-panel-body">
                      <div
                        className="employee-card-tabs employee-card-tabs-report-style"
                        role="tablist"
                        aria-label="Вкладки карточки организации"
                      >
                        <button
                          type="button"
                          role="tab"
                        ...
                        <button
                          type="button"
                          role="tab"
                          aria-selected={activeOrganizationCardTab === ORGANIZATION_CARD_TABS.STRUCTURE}
                          className={`employee-card-tab${
                            activeOrganizationCardTab === ORGANIZATION_CARD_TABS.STRUCTURE ? " active" : ""
                          }`}
                          onClick={() => {
                            if (isOrganizationCardEditMode) {
                              return;
                            }
                            setActiveOrganizationCardTab(ORGANIZATION_CARD_TABS.STRUCTURE);
                          }}
                          disabled={isOrganizationCardEditMode}
                        >
                          <IconListBulleted aria-hidden />
                        ...
                        <button
                          type="button"
                          role="tab"
                          aria-selected={activeOrganizationCardTab === ORGANIZATION_CARD_TABS.RELATIONS}
                          className={`employee-card-tab${
                            activeOrganizationCardTab === ORGANIZATION_CARD_TABS.RELATIONS ? " active" : ""
                          }`}
                          onClick={() => {
                            if (isOrganizationCardEditMode) {
                              return;
                            }
                            setActiveOrganizationCardTab(ORGANIZATION_CARD_TABS.RELATIONS);
                          }}
                          disabled={isOrganizationCardEditMode}
                        >
                          <IconLink aria-hidden />
                          <span>Связи организации</span>
                        </button>
                      </div>
                      ...
                      {activeOrganizationCardTab === ORGANIZATION_CARD_TABS.STRUCTURE && (
                        <div className="employee-card-main-tab-content organization-card-main-tab-content organization-card-structure-tab">
                          <div
                            ref={organizationStructureLayoutRef}
                            className="organization-card-structure-layout"
                          >
                            <div
                              className="organization-card-structure-pane organization-card-structure-pane--subdivisions"
                              style={{
                                flex: "0 0 auto",
                                width: `calc((100% - ${ORGANIZATION_STRUCTURE_PANES_HANDLE_PX}px) * ${sanitizeOrganizationStructurePanesSplit(organizationStructurePanesSplitFrac)})`,
                                minWidth: ORGANIZATION_STRUCTURE_PANES_MIN_LEFT_PX,
                                maxWidth: `calc(100% - ${ORGANIZATION_STRUCTURE_PANES_HANDLE_PX}px - ${ORGANIZATION_STRUCTURE_PANES_MIN_RIGHT_PX}px)`
                              }}
                            >
                              <section className="employee-card-section organization-card-structure-section">
                                <div className="employee-card-section-header">
                                  <h3>Подразделения</h3>
                                  <div className="employee-card-section-actions">
                                    <button
                                      type="button"
                                      className="panel-action-button"
                                      onClick={openOrganizationStructureAddModal}
                                    ...
                                    <button
                                      type="button"
                                      className="panel-action-button"
                                      onClick={handleRefreshOrganizationStructure}
                                      disabled={organizationStructureLoading}
                                      aria-label="Обновить список подразделений"
                                ...
                                {organizationStructureError ? (
                                  <p className="organization-card-structure-error" role="alert">
                                    {organizationStructureError}
                                  </p>
                                ) : null}
                                <div
                                  className="organization-card-structure-table-wrapper"
                                  ref={organizationStructureTableWrapperRef}
                                  onDragOver={(event) => {
                                    if (
                                      !structureDraggedOrganUnitIdRef.current &&
                                      !structureDraggedEmployeeOrganIdRef.current
                                    ) {
                                      return;
                                    }
                                ...
                                >
                                  <table className="organization-card-structure-table">
                                    <colgroup>
                                      <col
                                        style={{
                                          width: `${organizationStructureColWidthsPx.namePx}px`
                                        }}
                                      />
                                      <col
                                        style={{
                                          width: `${organizationStructureColWidthsPx.codePx}px`
                                        }}
                                      />
                                      <col
                                        style={{
                                          width: `${organizationStructureColWidthsPx.kcehPx}px`
                                        }}
                                      />
                                      <col
                                        style={{
                                          width: `${ORGANIZATION_STRUCTURE_ACTIONS_COL_PX}px`
                                        }}
                                      />
                                    </colgroup>
                                    <thead>
                                      <tr>
                                        <th className="organization-card-structure-th-resizable">
                                          <span className="organization-card-structure-th-label">Наименование</span>
                                          <span
                                            className="organization-card-structure-resize-handle"
                                            onMouseDown={(event) =>
                                              handleOrganizationStructureResizeStart("name-code", event)
                                            }
                                            role="separator"
                                            aria-hidden="true"
                                          />
                                        ...
                                        <th className="organization-card-structure-th-resizable">
                                          <span className="organization-card-structure-th-label">Код</span>
                                          <span
                                            className="organization-card-structure-resize-handle"
                                            onMouseDown={(event) =>
                                              handleOrganizationStructureResizeStart("code-kceh", event)
                                            }
                                            role="separator"
                                            aria-hidden="true"
                                          />
                                        ...
                                        <th className="organization-card-structure-th-resizable">
                                          <span className="organization-card-structure-th-label">Номер цеха</span>
                                          <span
                                            className="organization-card-structure-resize-handle"
                                            onMouseDown={(event) =>
                                              handleOrganizationStructureResizeStart("kceh-name", event)
                                            }
                                            role="separator"
                                            aria-hidden="true"
                                          />
                                        ...
                                        <th
                                          className="organization-card-structure-actions-col"
                                          aria-label="Действия"
                                        />
                                      </tr>
                                    </thead>
                                    <tbody>
                                      <tr
                                        className={`organization-card-structure-root-row organization-card-structure-row-selectable${
                                          structureDropTargetId === ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY
                                            ? " organization-card-structure-drop-target"
                                            : ""
                                        }${
                                          organizationStructureSelectedRowId &&
                                          organizationStructureSelectedRowId ===
                                            resolveSelectedStructureRootOrganUnitId()
                                            ? " organization-card-structure-row-selected"
                                            : ""
                                        }`}
                                        ...
                                        onClick={(event) => {
                                          ...
                                          const rootId = resolveSelectedStructureRootOrganUnitId();
                                          if (rootId) {
                                            handleOrganizationStructureRowSelect(rootId);
                                          }
                                        }}
                                        onDragOver={(event) =>
                                          handleOrganizationStructureRowDragOver(
                                            ORGANIZATION_STRUCTURE_DND_DROP_ROOT_KEY,
                                            event
                                          )
                                        }
                                        onDrop={(event) => {
                                          event.preventDefault();
                                          void handleOrganizationStructureParentDrop(
                                            resolveSelectedStructureRootOrganUnitId()
                                          );
                                        }}
                                      >
                                        <td
                                          className="organization-card-structure-name-cell organization-card-structure-ellipsis"
                                          onMouseEnter={(event) =>
                                            handleCellMouseEnter(
                                              event,
                                              organizationStructureRootDisplayName || "—"
                                        ...
                                        <td className="organization-card-structure-ellipsis" />
                                        <td className="organization-card-structure-ellipsis" />
                                        <td className="organization-card-structure-actions-cell" />
                                      </tr>
                                      {organizationStructureLoading &&
                                      organizationStructureFlatRows.length === 0 ? (
                                        <tr>
                                      ...
                                      {organizationStructureFlatRows.map((row) => (
                                        <tr
                                          key={row.organUnitChildId || row.organUnitHierarchy}
                                          className={`organization-card-structure-draggable-row organization-card-structure-row-draggable organization-card-structure-row-selectable${
                                            structureDraggingOrganUnitId === row.organUnitChildId
                                              ? " organization-card-structure-row-dragging"
                                              : ""
                                          }${
                                            structureDropTargetId === row.organUnitChildId
                                              ? " organization-card-structure-drop-target"
                                              : ""
                                          }${
                                            organizationStructureSelectedRowId === row.organUnitChildId
                                              ? " organization-card-structure-row-selected"
                                              : ""
                                          }`}
                                          aria-selected={
                                          ...
                                          draggable={!organizationStructureLoading}
                                          onClick={(event) => {
                                            if (
                                              event.target.closest?.(
                                                ".employee-card-position-action-button"
                                              ) ||
                                              event.target.closest?.(".organization-card-structure-tree-toggle")
                                            ) {
                                              return;
                                            }
                                            handleOrganizationStructureRowSelect(row.organUnitChildId);
                                          }}
                                          onDragStart={(event) => {
                                            if (
                                              event.target.closest?.(".organization-card-structure-tree-toggle") ||
                                            ...
                                            structureDraggedOrganUnitIdRef.current = row.organUnitChildId;
                                            setStructureDraggingOrganUnitId(row.organUnitChildId);
                                            event.dataTransfer.effectAllowed = "move";
                                            event.dataTransfer.setData("text/plain", row.organUnitChildId);
                                            const startX = Number(
                                              event?.clientX ?? event?.nativeEvent?.clientX
                                            );
                                            const startY = Number(
                                              event?.clientY ?? event?.nativeEvent?.clientY
                                            );
                                          ...
                                          onDrag={(event) => {
                                            if (!structureDraggedOrganUnitIdRef.current) {
                                              return;
                                            }
                                          ...
                                          onDragEnd={() => {
                                            stopOrganizationStructureDragAutoScroll();
                                            structureDraggedOrganUnitIdRef.current = "";
                                            setStructureDraggingOrganUnitId("");
                                            setStructureDropTargetId("");
                                          }}
                                          onDragOver={(event) =>
                                            handleOrganizationStructureRowDragOver(row.organUnitChildId, event)
                                          }
                                          onDrop={(event) => {
                                            event.preventDefault();
                                            void handleOrganizationStructureParentDrop(row.organUnitChildId);
                                          }}
                                        >
                                          <td
                                            className="organization-card-structure-name-cell"
                                            ...
                                            <div className="organization-card-structure-name-cell-inner">
                                              {row.hasChildren ? (
                                                <button
                                                  ...
                                                  aria-label={
                                                    row.isCollapsed ? "Развернуть вложенные" : "Свернуть вложенные"
                                                  }
                                                  onClick={(event) => {
                                                    event.stopPropagation();
                                                    toggleOrganizationStructureNodeCollapse(row.organUnitChildId);
                                                  }}
                                                >
                                                  {row.isCollapsed ? "▸" : "▾"}
                                                </button>
                                                ...
                                                <span
                                                  className="organization-card-structure-tree-toggle-spacer"
                                                  aria-hidden="true"
                                                />
                                              )}
                                              <span
                                                className="organization-card-structure-name-text"
                                                onMouseEnter={(event) =>
                                                  handleCellMouseEnter(event, row.organUnitName)
                                                }
                                                onMouseMove={updateCellTooltipPosition}
                                                onMouseLeave={handleCellMouseLeave}
                                              >
                                          ...
                                          <td
                                            className="organization-card-structure-ellipsis"
                                            onMouseEnter={(event) =>
                                              handleCellMouseEnter(event, row.organUnitCode)
                                            }
                                            onMouseMove={updateCellTooltipPosition}
                                            onMouseLeave={handleCellMouseLeave}
                                          >
                                          ...
                                          <td
                                            className="organization-card-structure-ellipsis"
                                            onMouseEnter={(event) =>
                                              handleCellMouseEnter(event, row.kcehNumber)
                                            }
                                            onMouseMove={updateCellTooltipPosition}
                                            onMouseLeave={handleCellMouseLeave}
                                          >
                                          ...
                                          <td className="organization-card-structure-actions-cell">
                                            <button
                                              type="button"
                                              className="employee-card-position-action-button"
                                              aria-label="Изменить подразделение"
                                              data-tooltip="Изменить"
                                              onClick={() => openOrganizationStructureEditModal(row.organUnitChildId)}
                                            >
                                              <IconPencil aria-hidden />
                                            </button>
                                            <button
                                              ...
                                              data-tooltip="Удалить"
                                              onClick={() => requestDeleteOrganizationSubdivision(row.organUnitChildId)}
                                            >
                                              <IconTrash aria-hidden />
                                            </button>
                                          </td>
                            ...
                            <div
                              className="organization-card-structure-panes-resizer"
                              onMouseDown={handleOrganizationStructurePanesResizeStart}
                              role="separator"
                              aria-orientation="vertical"
                              aria-label="Изменить ширину блоков Подразделения и Сотрудники организации"
                            />
                            <div
                              className="organization-card-structure-pane organization-card-structure-pane--employees"
                              style={{
                                flex: "1 1 0",
                                minWidth: ORGANIZATION_STRUCTURE_PANES_MIN_RIGHT_PX
                              }}
                            >
                              <OrganizationStructureEmployeesPanel
                                ref={organizationStructureEmployeesTableWrapRef}
                                rootOrganUnitId={resolveSelectedStructureRootOrganUnitId()}
                                rootOrganUnitName={organizationStructureRootDisplayName}
                                structureSubdivisionFilterSync={structureEmployeesSubdivisionFilterSync}
                                structureItems={organizationStructureItems}
                                headEmployees={organizationStructureHeadEmployees}
                                loading={organizationStructureLoading}
                                listEmployeesApiUrl={LIST_EMPLOYEES_API_URL}
                                listPositionsApiUrl={LIST_POSITIONS_API_URL}
                                employeePositionBaseUrl={`${ADMIN_API_BASE_URL}/employee-position`}
                                onRefresh={handleRefreshOrganizationStructure}
                                onEmployeeLinkUpsert={upsertOrganizationStructureEmployeeLink}
                                onEmployeeStructureDnDActiveChange={setEmployeeStructureDnDActive}
                                updateOrganizationStructureDragAutoScroll={
                                  updateOrganizationStructureDragAutoScroll
                                }
                                stopOrganizationStructureDragAutoScroll={
                                ...
                                showSystemErrorToast={showSystemErrorToast}
                                showSystemSuccessToast={showSystemSuccessToast}
                                getEmployeeCardHref={buildEmployeeCardUrl}
                              />
                            </div>
                          </div>
                        </div>
                      )}
                      {activeOrganizationCardTab === ORGANIZATION_CARD_TABS.RELATIONS && (
                        <div
                          className="employee-card-relations-tab-content organization-card-relations-tab-content"
                          ref={organizationCardRelationsTabContentRef}
                        >
                          <section className="employee-card-section">
                            <div className="employee-card-subordination-header">
                              <h3>Связи организации</h3>
                              <div className="employee-card-relations-header-actions">
                                <button
                                  type="button"
                                  className="panel-action-button"
                                ...
                                <ExportToExcelButton
                                  exportFile={requestOrganizationCardRelationsExport}
                                  disabled={organizationCardRelationsLoading}
                                  className="panel-action-button"
                                  onError={(error) =>
                                    showSystemErrorToast(
                                      error instanceof Error
                                        ? error.message
                                        : "Ошибка выгрузки связей организации"
                                    )
                                  }
                                />
                                <button
                                  type="button"
                                  className="panel-action-button"
                                  onClick={clearOrganizationCardRelationFilters}
                                >
                                  <IconClose aria-hidden />
                                  <span>Удалить фильтр</span>
                                </button>
                                <button
                                  type="button"
                                  className="panel-action-button"
                                  onClick={handleAlignOrganizationCardRelationColumns}
                                >
                                  <IconAlignColumns aria-hidden />
                                  <span>Выровнять</span>
                                </button>
                            ...
                            <div
                              className={`employee-card-relations-table-wrapper${
                                organizationCardRelationsLoading && organizationCardRelations.length > 0
                                  ? " employee-card-relations-table-wrapper--stale-refresh"
                                  : ""
                              }`}
                              ref={organizationCardRelationsTableWrapperRef}
                            >
                              <table
                                className="employee-card-relations-table"
                                style={{
                                  width: `${organizationCardRelationTableWidthPx}px`,
                                  minWidth: `${organizationCardRelationTableWidthPx}px`
                                }}
                              >
                                <colgroup>
                                  {ORGANIZATION_RELATION_COLUMNS.map((column) => (
                                    <col
                                      key={`org-relation-col-${column.key}`}
                                      style={{
                                        width: `${getOrganizationCardRelationColumnWidthPx(column.key)}px`,
                                        minWidth: `${getOrganizationCardRelationColumnWidthPx(column.key)}px`
                                      }}
                                    />
                                  ))}
                                  <col style={{ width: "96px", minWidth: "96px" }} />
                                ...
                                <thead>
                                  <tr>
                                    {ORGANIZATION_RELATION_COLUMNS.map((column) => {
                                      const direction = getOrganizationCardRelationSortDirectionForField(column.key);
                                      const sortOrder = getOrganizationCardRelationSortOrderForField(column.key);
                                      const sortIcon =
                                        direction === "ASC" ? "▲" : direction === "DESC" ? "▼" : null;
                                      return (
                                        <th
                                        ...
                                        >
                                          <button
                                            ...
                                            onClick={() => handleOrganizationCardRelationSortClick(column.key)}
                                          >
                                            <span>{column.title}</span>
                                            {sortIcon && (
                                              <span className="sort-icon-group">
                                          ...
                                          <span
                                            className="employee-card-relations-resize-handle"
                                            onMouseDown={(event) =>
                                              handleOrganizationCardRelationResizeStart(column.key, event)
                                            }
                                            role="presentation"
                                          />
                                        </th>
                                  ...
                                  <tr className="employee-card-relations-filter-row">
                                    {ORGANIZATION_RELATION_COLUMNS.map((column) => (
                                      <th
                                        key={`org-relation-filter-${column.key}`}
                                        className={
                                          column.key === "defaultFlag"
                                            ? "employee-card-relations-default-flag-column"
                                            : ""
                                      ...
                                      >
                                        {column.key === "defaultFlag" ? (
                                          <select
                                            className="employee-card-relations-filter-input"
                                            value={organizationCardRelationsFilters.defaultFlag}
                                            onChange={(event) =>
                                              handleOrganizationCardRelationFilterChange(
                                                "defaultFlag",
                                                event.target.value
                                              )
                                            }
                                          ...
                                          <div className="column-filter-input-wrapper">
                                            <input
                                              type="text"
                                              className="employee-card-relations-filter-input"
                                              value={organizationCardRelationsFilters[column.key] ?? ""}
                                              onChange={(event) =>
                                                handleOrganizationCardRelationFilterChange(
                                                  column.key,
                                                  event.target.value
                                            ...
                                            />
                                            {String(
                                              organizationCardRelationsFilters[column.key] ?? ""
                                            ).trim() !== "" && (
                                              <button
                                                type="button"
                                                className="column-filter-clear-button"
                                                aria-label="Очистить фильтр"
                                                onClick={() =>
                                                  handleOrganizationCardRelationFilterChange(column.key, "")
                                                }
                                              >
                                                ×
                                              </button>
                                ...
                                <tbody>
                                  {isAddingEmployeeRelation &&
                                    renderEmployeeRelationFormRow("Отменить добавление связи", "org-relation-add-row")}
                                  {organizationCardRelationsLoading &&
                                    organizationCardRelations.length === 0 && (
                                      <tr>
                                        <td colSpan={6}>Загрузка данных...</td>
                                      </tr>
                                    )}
                                  ...
                                  {organizationCardRelations.map((relationRow, index) => {
                                    if (getRelationId(relationRow) === editingEmployeeRelationId) {
                                      return renderEmployeeRelationFormRow(
                                        "Отменить редактирование связи",
                                        `org-relation-edit-${editingEmployeeRelationId || index}`
                                      );
                                    }
                                    const orgRelRowKey = getOrganizationCardRelationRowSelectionKey(relationRow);
                                    const isOrgRelRowSelected = selectedOrganizationCardRelationKey === orgRelRowKey;
                                    return (
                                        <tr
                                          key={`${getRelationId(relationRow) || "relation"}-${
                                            relationRow?.relationTypeId ?? "type"
                                          }-${index}`}
                                          className={
                                            isOrgRelRowSelected
                                              ? "organization-card-relations-row-selected"
                                              : undefined
                                          }
                                          aria-selected={isOrgRelRowSelected}
                                          onClick={(event) => {
                                            const el = event.target;
                                            if (el.closest?.("a[href]")) {
                                              return;
                                            ...
                                            setSelectedOrganizationCardRelationKey(orgRelRowKey);
                                          }}
                                          onMouseDown={() => {
                                            if (isRelationFormActive) {
                                              cancelAddEmployeeRelationRow();
                                        ...
                                        >
                                          <td
                                            onMouseEnter={(event) =>
                                              handleCellMouseEnter(
                                                event,
                                                relationRow?.employeeName ?? "-"
                                              )
                                            }
                                            onMouseMove={updateCellTooltipPosition}
                                            onMouseLeave={handleCellMouseLeave}
                                          >
                                            {normalizeEmployeeId(relationRow?.employeeId) ? (
                                              <a
                                                className="entity-card-hover-link employee-name-link"
                                                href={buildEmployeeCardUrl(relationRow?.employeeId)}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                onClick={(event) => {
                                                  event.stopPropagation();
        ...
        {organizationStructureEditor && (
          <Suspense fallback={null}>
            <OrganizationStructureEditorModal
              key={`${organizationStructureEditor.mode}-${organizationStructureEditor.organUnitChildId ?? "new"}-${organizationStructureEditor.initialParentOrganUnitId ?? ""}`}
              mode={organizationStructureEditor.mode}
              initialName={organizationStructureEditor.initialName}
              initialCode={organizationStructureEditor.initialCode}
              initialKcehNumber={organizationStructureEditor.initialKcehNumber}
              initialParentOrganUnitId={organizationStructureEditor.initialParentOrganUnitId}
              parentOptions={organizationStructureEditorParentSelectOptions}
              saving={organizationStructureEditorSaving}
              onClose={closeOrganizationStructureEditorModal}
              onSave={submitOrganizationStructureEditorModal}
            />
          </Suspense>
        )}
        {pendingOrganizationSubdivisionDelete && (
          <div className="modal-overlay" role="presentation">
            <div className="modal" role="dialog" aria-modal="true" aria-label="Удаление подразделения">
              <p className="result-message">
                Удалить подразделение
                {pendingOrganizationSubdivisionDelete.name
                  ? ` "${pendingOrganizationSubdivisionDelete.name}"`
                  : ""}
                ?
              </p>
              <div className="confirm-actions">
                <button type="button" className="modal-close-button" onClick={confirmDeleteOrganizationSubdivision}>
                  Да
                </button>
                <button
                  type="button"
                  className="modal-close-button secondary"
                  onClick={closeDeleteOrganizationSubdivisionModal}
                >
                  <IconClose aria-hidden />
                  <span>Нет</span>
                </button>
